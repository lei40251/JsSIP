CHANGELOG
=========
### 2.0.6

* 优化 Ai 降噪能力，完善多声道场景处理
* 优化视频码率与 SDP 处理逻辑，提升部分场景下的音视频稳定性
* 优化虚拟背景显示效果，提升部分场景下的兼容性与稳定性
* 新增 Base JS Demo 三方通话示例，支持成员呼入、静默接听和定向屏幕共享
* 优化 `RTCSession.share(type, options)` 对象参数，并新增非 BFCP 独立屏幕辅流能力
* 优化混流视频输出，降低混流延迟，兼容性差的环境自动降级
* 完善通话统计排障信息，默认每 10 秒记录一次脱敏后的原始 WebRTC 统计报告

### 2.0.5

* 优化虚拟背景效果，修复切换时原背景短暂显示问题
* 优化虚拟背景处理逻辑，提升部分场景下的兼容性与稳定性

### 2.0.4

* 新增镜像、水印、虚拟背景和Ai降噪等

### 1.13.5

* 兼容安卓微信和鸿蒙系统视频质量问题

### 1.13.3

* getEnvironmentId() 支持传入设备列表 devices

### 1.13.2

* 修改特定获取特定手机后摄ID的方法名为 getEnvironmentId()

### 1.13.1

* remoteSupportsVideo 改为延迟 emit，避免过早触发事件
* 优化保清晰、保流畅方法

### 1.13.0

* 新增SDK内部虚拟背景调用方法
* 虚拟背景增加镜像方法
* 优化语音转单向视频处理方式
* 增加虚拟背景相关日志
* 优化虚拟背景参数

### 1.12.1

* 优化虚拟背景调用
* 增加远端是否支持视频模式的事件 remoteSupportsVideo

### 1.12.0

* 优化特定场景音视频切换问题
* 增加支持虚拟背景

### 1.11.19

* 修复自定义流接听和拒绝视频场景bug
* 修复减少SDP相关日志

### 1.11.18

* 修复特定场景dtmf id问题
* 优化拒绝远端视频请求
* 特定场景呼手机183兼容问题

### 1.11.17

* 优化网络质量数据处理逻辑
* 新增断网重连信令异常错误,在网络异常后通话挂断原因增加 re-Invite Error
* 优化getStats的停止逻辑
* 修复Firefox音视频切换bug

### 1.11.16

* 增加通话前100信令事件

### 1.11.15

* 适配某些场景关闭视频再开以后卡顿问题

### 1.11.14

* 修复三星手机浏览器关键帧兼容问题

### 1.11.13

* 优化华为手机切换摄像头，兼容鸿蒙6系统浏览器

### 1.11.12

* 新增兼容 iOS15.x 等设备摄像头流黑屏情况下的事件通知

### 1.11.11

* 优化注册信令头信息

### 1.11.10

* 优化微信及iOS多麦克风处理逻辑

### 1.11.9

* 修复单向视频切换音频问题

### 1.11.8

* 优化重连后视频卡住问题
* 优化安卓微信confirmed事件触发时间
* 优化iOS多麦克风及日志

### 1.11.7

* 优化接通前多个SDP的适配
* SDK Demo增加默认前摄像头

### 1.11.6

* 优化切换摄像头适配特殊机型
* 优化网络质量统计返回6的逻辑
* 新增ua关于浏览器网络的事件

### 1.11.5

* 新增切后台且视频不发送时切换为黑图
* 优化微信浏览器特定场景无声音问题
* 优化混流时音频处理
* 优化切换摄像头逻辑

### 1.11.4

* 优化音频混流避免可能造成解码异常

### 1.11.3

* 修复特定场景视频切换原因变Hold的问题

### 1.11.2

* 修复安卓微信纯视频呼叫还包含音频的问题

### 1.11.1

* 修复切换摄像头facingMode与deviceId混用的问题

### 1.11.0

* 优化DC异常重试功能
* 优化生成空视频方法
* 修复自定义流相关BUG
* 新增设置保清晰/保流畅方法

### 1.10.9

* 新增BFCP支持

### 1.10.8

* 优化自定义流呼叫volte适配200ok无sdp的情况
* 优化进队列接通流程

### 1.10.7

* 优化重连后特殊情况下不发送相关信令问题

### 1.10.6

* 优化网络切换后收不到bye消息的问题
* 优化丢包和网络质量统计算法
* 优化默认呼叫H264编码设置
* 增加适配鸿蒙WebView首次呼叫使用听筒问题

### 1.10.5

* 优化断网重连
* 优化网络质量算法
* 新增适配不同版本SDP，call方法新增cMode参数

### 1.10.4

* 删除外呼的 urn:3gpp:video-orientation，以解决呼叫部分手机通话异常
* 修复修改SDP后信令头部内容长度值
* 优化手机183带视频的时候造成上行两个视频的问题
* 修复适配微信外呼造成的bug

### 1.10.3

* 修复firefox兼容问题
* 修复微信浏览器音频替换问题

### 1.10.2

* 日志输出curr实时轨道状态调整
* 分享桌面返回分享的实时流

### 1.10.1

* 新增信令重新协商场景：信令重连成功后会重新协商以保证媒体正常；开始通话前媒体端口一直不通的时候会重新协商媒体端口；

### 1.10.0

* 修复ice状态异常重新协商bug
* 优化SDK内部发送和接收SDP内容
* 修复统计信息显示里面的 mic 数量
* 恢复媒体轨道保持原来的muted状态
* 修复摄像头失效重新获取逻辑问题，修复媒体不通情况下的统计数据0改为null

### 1.9.12

* 如果使用sdk获取媒体流会启用适配iOS15.1&15.2的canvas
* 发起呼叫媒体不通 iceConnectionState == 'failed'后自动挂断通话并报媒体超时，如果已经正常通话过程中异常则自动重新协商
* 新增 peerconnection:iceConnectionState 事件，详见API文档 session 事件
* 日志:减少初始记录媒体属性为开始的一次，实时获取改为定时记录并间隔记录全量report日志

### 1.9.11

* 优化合并部分冗余日志

### 1.9.10

* session.mute() & session.unmute() 两个方法去掉判断当前状态，改为直接执行设置

### 1.9.9

* getStreams 新增响应参数 mediaStream，包含远端媒体音频及视频
* 新增通话结束自动释放媒体统计报告定时器
* 日志记录优化

### 1.9.8

* 新增ua.start()调用后5秒信令连接未建立事件

### 1.9.7

* videoTrackState 因为muted属性变化的触发条件，改为muted变化或者muted == true
* 日志记录优化

### 1.9.6

* 修复单向被叫bug
* 增加通话中的网络状态统计事件 network-quality, 调整了原来通话状态统计 report事件返回参数
* 增加通话前网络检测示例
* 增加了通话开始5秒左右输出一次完整getStats Report

### 1.9.5

* 修复增加mid造成被叫无SDP问题.

### 1.9.4

* 解决部分手机呼叫无法接通提示488问题.
* SDK demo 增加turn配置，解决488问题手机偶现媒体不通问题.
