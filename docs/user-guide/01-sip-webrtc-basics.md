...
呼出：页面调用 `ua.call()`，SDK先准备本地媒体和 SDP Offer，再发送 INVITE，所以 Promise 可能因媒体权限/约束失败而 reject。

呼入：SDK先收到远端 INVITE 并产生 `newRTCSession`，用户尚未接听，因此本地摄像头/麦克风和 PeerConnection 可能还未创建。调用 `session.answer(options)` 后才准备本地媒体并发送 SDP Answer。
...
普通接入不要直接正则修改 SDP。确有互通要求时，应明确修改的媒体行、方向、编码或带宽字段，并覆盖呼出、呼入、UPDATE/re-INVITE、共享和重协商路径。
...
- 不要提前 stop 一个仍被通话 sender 使用的自定义 track。

## 1.12 本地预览和实际发送不是同一件事

`localVideo.srcObject = stream` 只是把某个流显示在页面，不代表该流一定已经发送。实际发送取决于 RTCSession 的 sender、当前协商方向和 track 状态。
...
```text
麦克风 ── AiNS ─────────────┐
                            ├─ MediaEffectsComposer/媒体管线 ─→ RTCSession sender
摄像头 ── 虚拟背景(slot 0) ─┤
其他音视频源(slot 1...) ────┘
```

- AiNS 处理本地音频。
- 虚拟背景作用于 composer 的某一路视频源，通话摄像头通常是 slot 0。
- composer 可处理镜像、水印和多路输入。
- 这些能力通过 `ua.call()` / `session.answer()` 的 options 初始化，并从当前 session 获取控制器动态调整。
- 效果失败时应尽量保持基础通话，不要把效果异常当作 SIP 通话失败。
...
例如“对方说我声音断断续续”优先看上行音频；“我听对方断断续续”优先看下行音频。网络质量 `0` 表示暂无有效样本，`1` 最好，`6` 最差或连接不可用，详细阈值见第 5 章。
