# MediaStreamComposer 渲染后端升级方案

## 背景

当前 `lib/MediaStreamComposer.js` 使用主线程 Canvas 2D 完成视频混合：
...
### MediaStreamComposer 主类职责

`MediaStreamComposer` 保持负责：
...
- 因此 `MediaStreamComposer.js` 需要改为延迟初始化 canvas context，由 renderer 决定拿 2D、WebGL2 还是 offscreen。
...
这是当前 `MediaStreamComposer.js` 的现有逻辑，应保留并作为最终兜底。
...
- `MediaStreamComposer.js` 仍默认使用 `MainCanvas2DRenderer`。
- 保证行为完全不变。
- 跑通 lint/test/demo。
...
```js
new CRTC.MediaStreamComposer([stream]).getMixedStream();
```
