# MediaStreamComposer.js — 主控制器详解

**文件**: [lib/MediaStreamComposer/Core/MediaStreamComposer.js](../lib/MediaStreamComposer/Core/MediaStreamComposer.js)

MediaStreamComposer 采用**调解者模式**，不直接处理渲染、音频、布局等具体逻辑，而是协调 6 个子模块完成工作：
...
**设计意图**: MediaStreamComposer 只决定"每路视频画在哪里"，具体的"怎么画"交给渲染器。
...
**注意**: `stop()` 后同一个实例不可再使用，必须 `new MediaStreamComposer()`。
...
MediaStreamComposer 在各子模块构造时注入回调，形成数据流：
...
MediaStreamComposer 构造时就创建 6 个子模块，好处：
...
MediaStreamComposer 约 95% 的私有方法是一行委派调用。这是有意为之：
- **职责单一**：控制器只负责编排，不负责实现
- **可测试性**：每个子模块可以独立测试
