# MediaStreamComposer — 音视频合成模块

将多路 `MediaStream` / `HTMLMediaElement` 合并为单路音视频输出流。
视频使用 Canvas 合成（支持 WebGL2 / Canvas2D 多后端）；音频通过 WebAudio API 混音。

**主入口**: [lib/MediaStreamComposer.js](../lib/MediaStreamComposer.js)  
**核心**: [lib/MediaStreamComposer/Core/MediaStreamComposer.js](../lib/MediaStreamComposer/Core/MediaStreamComposer.js)  
**兼容入口**: [lib/Mixer.js](../lib/Mixer.js)  
**导出**: `class MediaStreamComposer`，并兼容旧名 `Mixer`
...
1. [模块结构](#1-模块结构)
2. [核心概念](#2-核心概念)
3. [Core — 核心子模块](#3-core--核心子模块)
4. [Renderers — 渲染后端](#4-renderers--渲染后端)
...
MediaStreamComposer 分为两个子模块：

```
lib/
├── MediaStreamComposer.js                 # 主入口
├── Mixer.js                               # 旧入口兼容层
├── MediaStreamComposer/
│   ├── Core/                              # 核心逻辑：源管理、布局、音频、输出
│   │   ├── MediaStreamComposer.js         # 主控制器，调解者模式协调各子模块
...
**Core** 负责业务逻辑：源管理、布局计算、音频混音、输出流管理。  
**Renderers** 负责将布局数据绘制到 Canvas，提供多种渲染后端并支持自动降级。
...
**状态委派**: `MediaStreamComposer` 不持有子模块状态的副本。19 个旧私有属性（`_sources`、`_renderer`、`_audioContext` 等）通过 `Object.defineProperty` 只读 getter 直接委派到子模块，无需手动同步。
...
### 3.1 MediaStreamComposer（主控制器）

调解者模式，串联所有子模块。主要流程：

```
new MediaStreamComposer(videos, options)
...
**`stop()` 后**: 同一实例不再可用，需 `new MediaStreamComposer()`。
...
```
调用方                              MediaStreamComposer
  │                                      │
  │  new MediaStreamComposer([streamA])                │
  │ ──────────────────────────────────►  │
...
各子模块源码含详细注释：
- **Core**: [SourceRegistry.js](../lib/MediaStreamComposer/Core/SourceRegistry.js)、[LayoutEngine.js](../lib/MediaStreamComposer/Core/LayoutEngine.js)、[AudioMixer.js](../lib/MediaStreamComposer/Core/AudioMixer.js)、[OutputStreamManager.js](../lib/MediaStreamComposer/Core/OutputStreamManager.js)、[RenderLoop.js](../lib/MediaStreamComposer/Core/RenderLoop.js)、[MixerConfig.js](../lib/MediaStreamComposer/Core/MixerConfig.js)
- **Renderers**: [RendererFactory.js](../lib/MediaStreamComposer/Renderers/RendererFactory.js)、[MainCanvas2DRenderer.js](../lib/MediaStreamComposer/Renderers/MainCanvas2DRenderer.js)、[MainWebGL2Renderer.js](../lib/MediaStreamComposer/Renderers/MainWebGL2Renderer.js)、[WorkerRenderer.js](../lib/MediaStreamComposer/Renderers/WorkerRenderer.js)
