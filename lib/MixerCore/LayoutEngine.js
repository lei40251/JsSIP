...
class LayoutEngine
{
  ...
  createRenderPayload()
  {
    ...
    {
      this._logger.debug(
        `Render payload created: size=${payload.width}x${payload.height} items=${items.length} sourceWatermarks=${payload.sourceWatermarks.length} outputWatermarks=${payload.outputWatermarks.length}`
      );
    }
