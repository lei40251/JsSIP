...
module.exports = class ReferSubscriber extends EventEmitter
{
  ...
  sendRefer(target, options = {})
  {
    ...
    const request = this._session.sendRequest(CRTC_C.REFER, {
      extraHeaders,
      eventHandlers : {
        ...
        {
          this._requestFailed(response, CRTC_C.causes.REJECTED);
        },
        onTransportError : () =>
        {
          this._requestFailed(null, CRTC_C.causes.CONNECTION_ERROR);
        },
        onRequestTimeout : () =>
        {
          this._requestFailed(null, CRTC_C.causes.REQUEST_TIMEOUT);
        },
        onDialogError : () =>
        {
          this._requestFailed(null, CRTC_C.causes.DIALOG_ERROR);
        }
      }
    });
