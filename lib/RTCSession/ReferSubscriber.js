...
const debug = require('debug')('FlyInn:RTCSession:ReferSubscriber');
...
module.exports = class ReferSubscriber extends EventEmitter
{
  ...
  receiveNotify(request)
  {
    ...
    if (status_line === -1)
    {
      ...
      log.debug(`receiveNotify() | error parsing NOTIFY body: "${request.body}"`);
  ...
  _requestSucceeded(response)
  {
    ...
    log.debug('REFER succeeded');

    debug('emit "requestSucceeded"');
    log.debug('emit "requestSucceeded"');
  ...
  {
    debug('REFER failed');
    log.debug('REFER failed');

    debug('emit "requestFailed"');
    log.debug('emit "requestFailed"');
