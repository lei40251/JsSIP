...
{
  return require('../MediaEffectsComposer/MediaEffectsComposer');
}

function getAiNSEngineCtor()
{
  return require('../AiNoiseSuppression/AiNSEngine');
}
...
const MEDIA_EFFECTS_COMPOSER_IMMUTABLE_FIELDS = [
  'width',
  'height',
  'fps',
  'renderMode',
...
function cloneComposerSessionOptions(options)
{
  ...
  const cloned = clonePlainObject(options);
  const normalizedSources = normalizeComposerSourceList(options);

  if (normalizedSources.length > 0)
  {
    cloned.sources = normalizedSources;
  }
  else
  {
    delete cloned.sources;
  }

  return cloned;
}

function mergeComposerSourceOptions(baseSource, patchSource)
{
  if (!baseSource && !patchSource)
  {
    return null;
  }

  const merged = Object.assign({}, clonePlainObject(baseSource), clonePlainObject(patchSource));

  if (!Object.prototype.hasOwnProperty.call(merged, 'aiBackground'))
  {
    if (baseSource && Object.prototype.hasOwnProperty.call(baseSource, 'aiBackground'))
    {
      merged.aiBackground = baseSource.aiBackground;
    }
  }
  else if (merged.aiBackground && typeof merged.aiBackground === 'object')
  {
    merged.aiBackground = clonePlainObject(merged.aiBackground);
  }
...
module.exports = class MediaPipeline
{
   ...
   * 它通过此方法将 MediaEffectsComposer._onIssue 回调与 RTCSession._emitMediaEffectsIssue
   * 桥接起来，完成跨模块的异常上报。
   *
   * ## 调用链
   *
   *   MediaEffectsComposer._onIssue（或 AiNSEngine._onIssue）
  ...
  getAiVirtualBackground()
  {
    ...
    if (composer && typeof composer.getAiBackground === 'function')
    {
      try
      {
        return composer.getAiBackground(0);
      }
      catch (error)
      {}
    }
    ...
    return sourceOptions && sourceOptions.aiBackground ? sourceOptions.aiBackground : null;
  }
  ...
  /**
   * 合并 RTCSession 侧缓存的 composer 视图。
   *
   * 这个缓存是显式的“会话态”，只保证：
   * - source 0（本端主源）的 sourceMirror / aiBackground；
   * - 输出层配置（mirror / watermark 等）；
   * - 为重建 composer 所需的构造参数（width / height / fps / renderMode...）。
   *
   * 业务若直接在 composer 实例上 addSource/removeSource/setConfig 多源扩展，
   * 这些动态实例态不会被镜像回 RTCSession，会话重建时需要业务自行重放。
   *
   * @param {Object|null} previousOptions
   * @param {Object|null} nextOptions
   * @returns {Object|null}
   */
  mergeSessionMediaEffectsComposerOptions(previousOptions, nextOptions)
  {
    const previous = cloneComposerSessionOptions(previousOptions) || {};
    const next = cloneComposerSessionOptions(nextOptions);
    ...
    const previousSources = normalizeComposerSourceList(previous);
    const nextHasSources = Object.prototype.hasOwnProperty.call(next, 'sources');
    const nextSources = normalizeComposerSourceList(next);

    if (nextHasSources)
    {
      const mergedPrimarySource = mergeComposerSourceOptions(previousSources[0], nextSources[0]);

      if (mergedPrimarySource)
      {
        merged.sources = [ mergedPrimarySource ];
      }
      else
      {
    ...
    else if (previousSources.length > 0)
    {
      merged.sources = previousSources;
    }

    return merged;
  }

  getImmutableMediaEffectsComposerFields(options)
  {
    if (!options || typeof options !== 'object')
    {
      return [];
    ...
    return MEDIA_EFFECTS_COMPOSER_IMMUTABLE_FIELDS.filter((field) =>
    {
  ...
  /**
   * 将 session 入参 shape 转成运行时 patch，并显式拒绝不可热更新字段。
   *
   * @param {Object|null} options
   * @returns {Object}
   */
  resolveMediaEffectsComposerPatch(options)
  {
    const patch = cloneComposerSessionOptions(options) || {};
    const immutableFields = this.getImmutableMediaEffectsComposerFields(patch);

    if (immutableFields.length > 0)
    {
      throw new TypeError(
        `updateMediaEffectsComposer() does not support runtime updates for: ${immutableFields.join(', ')}`
      );
    }
   ...
   * 5. 记录输入流引用（用于后续 replaceTrack 场景）。
   *
  ...
  async applyAiNoiseSuppressionOnSdkGumStream(stream, aiNSOptions)
  {
    ...
    const generation = (session._aiNSOperationGeneration || 0) + 1;
    const previousEngine = session._sessionAiNSEngine;
    const previousInputStream = session._aiNSInputStream;
    let engine = null;
    ...
    try
    {
      ...
      const processedStream = await engine.process(stream);
    ...
    catch (error)
    {
      ...
      if (engine && typeof engine.destroy === 'function')
      {
        try { await engine.destroy(); }
        catch (destroyError) { logger.warn(`${session._id} destroy failed ai noise suppression engine:`, destroyError); }
      }
   ...
   * - 轻量路径：复用现有引擎 → `engine.replaceTrack(stream)`，
   *   避免重新初始化 WASM 的开销（典型场景：通话中切换麦克风）。
   *
   * **降级链：**
   * - 引擎不存在 → 退化为完整初始化路径；
   * - replaceTrack 失败 → 停止旧引擎 → 退化为完整初始化路径。
   *
  ...
  async replaceAudioTrackWithSessionAiNoiseSuppression(stream, aiNSOptions = this._session._sessionAiNSOptions)
  {
    ...
    try
    {
      ...
      const processedStream = await session._sessionAiNSEngine.replaceTrack(stream);
   ...
   * 4. 如果源流包含音频，则获取 mixed 输出；否则获取 video 输出；
   * 5. 返回 composer 输出轨道构成的新流。
   *
   * **失败降级：** catch 后清理 composer、返回原流。
   *
   * @param {MediaStream} stream — 源媒体流。
   * @param {Object} composerOptions — composer 配置。
   * @returns {Promise<MediaStream>} — 合成后的流（包含 composer 输出的视频轨，以及存在时的混音音轨），
   *          或原流（如果无视频轨/处理失败）。
   */
  async applyMediaEffectsComposerOnSdkGumStream(stream, composerOptions, runtimeOptions)
  {
    ...
    const hasSourceAudio = Boolean(stream.getAudioTracks && stream.getAudioTracks().length > 0);
    let composer = null;
    ...
    try
    {
      ...
      const composerIssueHandler = this.emitMediaEffectsIssue.bind(this);
      ...
      const composerOutputStream = await composer.getOutput({
        type : hasSourceAudio ? 'mixed' : 'video'
      });
      const mixedVideoTrack = composerOutputStream && composerOutputStream.getVideoTracks ?
        composerOutputStream.getVideoTracks()[0] :
        null;
      ...
      if (composerOutputStream && composerOutputStream.getAudioTracks)
      {
        composerOutputStream.getAudioTracks().forEach((track) =>
        {
      ...
      try
      {
        // Composer 输出轨按普通视频轨处理，避免后续被按 detail/text 方向优化。
        mixedVideoTrack.contentHint = 'video';
      }
      catch (error)
      {
        logger.warn(`${session._id} set composer output contentHint failed: ${error.message || String(error)}`);
      }
  ...
  {
    const session = this._session;
    const nextTrack = change && change.track;
    const previousTrack = change && change.previousTrack;
    ...
    const localStream = session._localMediaStream;
    ...
    if (localStream && localStream.getVideoTracks && localStream.addTrack)
    {
      ...
      existingVideoTracks.forEach((track) =>
      {
        ...
        {
          try { localStream.removeTrack(track); }
          catch (error) {}
        }
      });
    ...
    const senders = session._connection && typeof session._connection.getSenders === 'function' ?
      session._connection.getSenders() : [];
    const videoSender = senders.find((sender) => sender && sender.track && sender.track.kind === 'video');
