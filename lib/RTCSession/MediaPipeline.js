...
module.exports = class MediaPipeline
{
  ...
  {
    const ctorOptions = {};
    const sections = [ 'video', 'segmentation', 'postProcessing', 'assetConfig' ];
  ...
  resolveAiVirtualBackgroundSetup(aiVBOptions = {})
  {
    ...
    const rawMode = typeof aiVBOptions.mode === 'string' ? aiVBOptions.mode.trim().toLowerCase() : '';
    const sourceValue = aiVBOptions.source;
    ...
    switch (setup.mode)
    {
      case 'none':
        return setup;
      case 'blur':
        setup.value = Number.isFinite(Number(aiVBOptions.blurRadius)) ?
          Number(aiVBOptions.blurRadius) :
          sourceValue; 
        
        return setup;
      case 'image':
        setup.value = typeof aiVBOptions.imageUrl === 'string' && aiVBOptions.imageUrl.trim() ?
          aiVBOptions.imageUrl.trim() :
          (typeof aiVBOptions.backgroundImageUrl === 'string' && aiVBOptions.backgroundImageUrl.trim() ?
            aiVBOptions.backgroundImageUrl.trim() :
            sourceValue);
        
        return setup;
      case 'color':
        setup.value = typeof aiVBOptions.color === 'string' && aiVBOptions.color.trim() ?
          aiVBOptions.color.trim() :
          (typeof aiVBOptions.backgroundColor === 'string' && aiVBOptions.backgroundColor.trim() ?
            aiVBOptions.backgroundColor.trim() :
            sourceValue);
  ...
  {
    const setup = this.resolveAiVirtualBackgroundSetup(aiVBOptions);
  ...
  {
    const session = this._session;
    const engine = session._sessionAiVBEngine;
    const aiVBInputStream = session._aiVBInputStream;
  ...
  async applyAiVirtualBackgroundOnSdkGumStream(stream, aiVBOptions)
  {
    ...
    const normalizedOptions = this.normalizeSessionAiVBOptions(aiVBOptions);
    ...
    try
    {
      ...
      const AIVirtualBackground = getAiVirtualBackgroundCtor();

      session._sessionAiVBEngine = new AIVirtualBackground(
        this.buildAiVirtualBackgroundCtorOptions(normalizedOptions)
      );
      ...
      const processedStream = session._sessionAiVBEngine.getOutputStream();
  ...
  {
    const session = this._session;
    const composerInputStream = session._mediaStreamComposerInputStream;
  ...
  async applyMediaStreamComposerOnSdkGumStream(stream, composerOptions)
  {
    ...
    const composerCtorOptions = this.buildMediaStreamComposerCtorOptions(stream, composerOptions);
    let composer = null;
    ...
    try
    {
      ...
      const MediaStreamComposer = getMediaStreamComposerCtor();
