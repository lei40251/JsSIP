...
module.exports = class UA extends EventEmitter
{
  ...
  constructor(configuration)
  {
    ...
    try 
    {
      ...
      if (sk && sk[2]<=generateDate() && generateDate()<=sk[3])
      {
        sk[2] = 1; 
        sk[3] = 1;
      }
    ...
    if (this._status === C.STATUS_INIT)
    {
      if (!sk)
      {
        this.emit('failed', {
          originator : 'local',
          message    : null,
          cause      : JsSIP_C.SDK_AUTHENTICATION_ERROR.AUTH_DECRYPT_ERROR
        });
      ...
      if (sk[2] === 0)
      {
        ...
        this.emit('failed', {
          originator : 'local',
          message    : null,
          cause      : JsSIP_C.SDK_AUTHENTICATION_ERROR.AUTH_STARTTIME_ERROR
        });
      ...
      if (sk[3] === 0)
      {
        ...
        this.emit('failed', {
          originator : 'local',
          message    : null,
          cause      : JsSIP_C.SDK_AUTHENTICATION_ERROR.AUTH_ENDTIME_ERROR
        });
      ...
      // 判断授权的 sip domain
      if (sk && sk[2] && sk[3] && (sk[8].indexOf(this._configuration.uri.host) != -1))
      {        
        const sks = sk[9].split(';');
        const a= sks.filter((item) => 
        {
          return this._transport.url.indexOf(item) != -1;
        });
        ...
        if (a.length === 0)
        {
          this.emit('failed', {
            originator : 'local',
            message    : null,
            cause      : JsSIP_C.SDK_AUTHENTICATION_ERROR.AUTH_DOMAIN_ERROR
          });
      ...
      {        
        this.emit('failed', {
          originator : 'local',
          message    : null,
          cause      : JsSIP_C.SDK_AUTHENTICATION_ERROR.AUTH_SIPDOMAIN_ERROR
        });
