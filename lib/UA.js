...
const debug = require('debug')('FlyInn:UA');
const debugerror = require('debug')('FlyInn:ERROR:UA');
...
module.exports = class UA extends EventEmitter
{
  ...
  start()
  {
    ...
    else if (this._status === C.STATUS_READY)
    {
      ...
      log.debug('UA is in READY status, not restarted');
    }
    else
    {
      debug('ERROR: connection is down, Auto-Recovery system is trying to reconnect');
      log.debug('ERROR: connection is down, Auto-Recovery system is trying to reconnect');
    }
  ...
  get(parameter)
  {
    switch (parameter)
    {
      ...
      default:
        ...
        log.error(`get() | cannot get "${parameter}" parameter in runtime`);
  ...
  set(parameter, value)
  {
    switch (parameter)
    {
      ...
      default:
        ...
        log.debug(`set() | cannot set "${parameter}" parameter in runtime`);
  ...
  receiveRequest(request)
  {
        ...
        request.ruri.user !== this._contact.uri.user)
    {
      ...
      log.debug('Request-URI does not point to us');
      ...
      switch (method)
      {
        case JsSIP_C.INVITE:
          ...
          else
          {
            ...
            log.error('INVITE received but WebRTC is not supported');
            request.reply(488);
          }
          break;
        case JsSIP_C.BYE:
        ...
        case JsSIP_C.CANCEL:
          ...
          else
          {
            ...
            log.debug('received CANCEL request for a non existent session');
          }
          break;
        case JsSIP_C.ACK:
          /* Absorb it.
    ...
    else
    {
      ...
      else if (method === JsSIP_C.NOTIFY)
      {
        ...
        else
        {
          ...
          log.debug('received NOTIFY request for a non existent subscription');
  ...
  _loadConfig(configuration)
  {
    ...
    this._configuration.flyinn_id = Utils.createRandomToken(5);
    ...
    {
      this._configuration.contact_uri = new URI('sip', this._configuration.uri.user, this._configuration.via_host, null, { transport: 'ws' });
    }
    ...
    log.debug('configuration parameters after validation:');
    for (const parameter in this._configuration)
    {
      ...
      if (Object.prototype.hasOwnProperty.call(config.settings, parameter))
      {
        switch (parameter)
        {
          ...
          case 'registrar_server':
            ...
            log.debug(`- ${parameter}: ${this._configuration[parameter]}`);
            break;
          case 'password':
          case 'ha1':
            debug(`- ${parameter}: NOT SHOWN`);
            log.debug(`- ${parameter}: NOT SHOWN`);
            break;
          default:
            debug(`- ${parameter}: ${JSON.stringify(this._configuration[parameter])}`);
            log.debug(`- ${parameter}: ${JSON.stringify(this._configuration[parameter])}`);
        }
      }
    }
