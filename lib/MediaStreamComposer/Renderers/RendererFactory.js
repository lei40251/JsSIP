...
exports.createRenderer = function(canvas, config, hooks)
{
  ...
  const hasSourceAiVirtualBackground = config.hasSourceAiVirtualBackground === true;
  const errors = [];
  ...
  if (
    ...
    (
      ...
      mode === 'worker-2d' ||
      mode === 'auto' ||
      (hasSourceAiVirtualBackground && mode === 'main-webgl2')
    )
  )
  {
    try
    {
      const workerMode = (mode === 'auto' || (hasSourceAiVirtualBackground && mode === 'main-webgl2')) ?
        (hasSourceAiVirtualBackground ? 'worker-2d' : 'worker-webgl2') :
        mode;
