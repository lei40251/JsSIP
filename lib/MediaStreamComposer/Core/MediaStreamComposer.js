...
const AudioComposer = require('./AudioComposer');
const OutputStreamManager = require('./OutputStreamManager');
const RenderLoop = require('./RenderLoop');
const MediaStreamComposerConfig = require('./ComposerConfig');
const ComposerDomAdapter = require('./ComposerDomAdapter');
const SourceAiVBManager = require('./SourceAiVBManager');
const WatermarkManager = require('./WatermarkManager');
const logger = new Logger('MediaStreamComposer');
 ...
 *   `aiVirtualBackground` 等。
  ...
  constructor(videos = [], options = {})
  {
    ...
    const config = MediaStreamComposerConfig.create(options);
  ...
  {
    const normalized = MediaStreamComposerConfig.normalizeSourceOptions(optionsOrSlot, index, this._config.audioGain);
  ...
  _resolveMirrorX(source, slot)
  {
    ...
    const hasSlotOverride = Object.prototype.hasOwnProperty.call(this._slotMirrorXOverrides, key);
  ...
  {
    const shouldForceMain2D = this._requiresMain2DRenderer();
    const shouldForceMainThread =
      this._isMirrorEnabled() ||
      this._isOutputMirrorEnabled() ||
      shouldForceMain2D;
    const previousPolicy = this._config.forceMainThreadRenderer;
    const previousMain2DPolicy = this._config.forceMain2DRenderer;
    ...
    const info = this._renderer.getInfo();
  ...
  _getSourceMirrorLegacySnapshot(slot)
  {
    ...
    const overrides = Object.keys(this._slotMirrorXOverrides).reduce((snapshot, key) =>
    {
    ...
    const normalizedSlot = this._normalizeSlot(slot, 0);
    ...
    const hasOverride = Object.prototype.hasOwnProperty.call(this._slotMirrorXOverrides, key);
    const override = hasOverride ? this._slotMirrorXOverrides[key] : null;
  ...
  async _getMixedOutput()
  {
    ...
    const mixedAudioStream = await this._audioComposer.getAudioStream();
  ...
  addSource(videos, optionsOrSlot)
  {
    ...
    const maxSources = MediaStreamComposerConfig.getMaxSources();
  ...
  async setConfig(patch)
  {
    ...
    if (patch.sourceMirrorOverrides && typeof patch.sourceMirrorOverrides === 'object')
    {
      ...
      {
        const normalizedSlot = this._normalizeSlot(slotKey, 0);
  ...
  setSourceAiVirtualBackground(slotOrTarget, options)
  {
    ...
    const source = this._resolveSourceForEffectUpdate(slotOrTarget);
  ...
  setMirror(enabled)
  {
    ...
    this.setConfig({ outputMirror: enabled });
  }
  ...
  setMirrorWatermarksWithOutput(enabled)
  {
    ...
    this.setConfig({ mirrorWatermarksWithOutput: enabled });
  }
  ...
  setSourceMirror(slotOrEnabled, enabled)
  {
    ...
    { 
      this.setConfig({ sourceMirror: slotOrEnabled });
  ...
  clearSourceMirror(slot)
  {
    ...
    if (slot === undefined || slot === null)
    {
      this.setConfig({ clearSourceMirrorOverrides: true });
