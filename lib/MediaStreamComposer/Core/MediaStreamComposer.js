...
const MediaStreamComposerConfig = require('./MixerConfig');
const ComposerDomAdapter = require('./MixerDomAdapter');
...
class MediaStreamComposer
{
  ...
  removeStream(streamOrId)
  {
    ...
    const removed = this._removeSource(this._findSource(streamOrId));
  ...
  setSourceMirror(slotOrEnabled, enabled)
  {
    ...
    const normalizedSlot = this._normalizeSlot(slotOrEnabled, 0);
  ...
  releaseSubmixAudioStream(options)
  {
    ...
    const released = this._audioMixer.releaseSubmixAudioStream(options);
