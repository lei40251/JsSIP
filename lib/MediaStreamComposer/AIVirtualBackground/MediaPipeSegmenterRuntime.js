...
module.exports = class MediaPipeSegmenterRuntime
{
  ...
  async initialize(options = {})
  {
    ...
    this.initializingPromise = (async() =>
    {
      ...
      const segmenter = await this.createSegmenterWithFallback(
        ImageSegmenter,
        vision,
        requestedDelegate
      );
  ...
  {
    const delegates = requestedDelegate === 'CPU' ? [ 'CPU' ] : [ requestedDelegate, 'CPU' ];
    let lastError = null;

    for (let index = 0; index < delegates.length; index += 1)
    {
      ...
      catch (error)
      {
        ...
        {
          logger.warn(
            `ImageSegmenter init failed with delegate ${delegate}, retrying with ${delegates[index + 1]}: ${
