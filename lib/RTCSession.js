...
module.exports = class RTCSession extends EventEmitter 
{
  ...
  async updateMediaEffectsComposer(options)
  {
    ...
    const patch = this._mediaPipeline.resolveMediaEffectsComposerRuntimePatch(resolvedOptions);
  ...
  _emitMediaEffectsIssue(issue)
  {
    ...
    const loggerMethod = logger[issue && issue.severity] || logger.warn;
    const payload = {
    ...
    loggerMethod.call(
      logger,
      `${this._id} emit "mediaeffectsissue": module=${normalizedIssue.module} message=${normalizedIssue.message}`
    );
