  ...
  connect(target, options = {}, initCallback)
  {
    ...
    if (this._ua.sk && (this._ua.sk[8].split(';').indexOf(target.host) == -1))
    {
      this._ua.emit('failed', {
        originator : 'local',
        message    : null,
        cause      : CRTC_C.causes.AUTHORIZATION_ERROR
      });
        ...
        .then((stream) =>
        {
          ...
          this._localShareStream.getVideoTracks()
            .forEach((track) =>
            {
              ...
              this.emit('cameraChanged', stream);
            });
        ...
        if (type === 'offer')
        {
          ...
          sdp.media.forEach((media) =>
          {
            if (media.type === 'video')
            {
              ...
              if (media.fmtp)
              {
                media.fmtp = media.fmtp.filter((r) => payloadArr.indexOf(r.payload) !== -1);
              }

              if (media.rtp)
              {
                media.rtp = media.rtp.filter((r) => payloadArr.indexOf(r.payload) !== -1);
              }
              
              if (media.rtcpFb)
              {
                media.rtcpFb = media.rtcpFb.filter((r) => payloadArr.indexOf(r.payload) !== -1);
              }
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then((stream) => 
      {
        if (stream)
        {
          ...
          // this._rtcAnswerConstraints['iceRestart'] = true;
        }
      })
      // Create local description.
      .then(() =>
