...
module.exports = class RTCSession extends EventEmitter 
{
  ...
  async updateMediaStreamComposer(options)
  {
    ...
    const resolvedOptions = this._mediaPipeline.resolveMediaStreamComposerOptions({
    ...
    const composer = this.getMediaStreamComposer();
    const shouldRecreateComposerForEffectRouteChange = Boolean(
      composer &&
      typeof composer.getRenderInfo === 'function' &&
      this._sessionMediaStreamComposerOptions &&
      this._sessionMediaStreamComposerOptions.sources instanceof Array &&
      this._sessionMediaStreamComposerOptions.sources.some((item) => item && item.aiVirtualBackground) &&
      (() =>
      {
        try
        {
          const renderInfo = composer.getRenderInfo() || {};
    ...
    {
      const sender = this._connection && typeof this._connection.getSenders === 'function' ?
        this._connection.getSenders().find((item) => item && item.track && item.track.kind === 'video') :
        null;
      const composerInputStream = this._mediaStreamComposerInputStream;
      const currentLocalVideoTrack = sender && sender.track ? sender.track :
      ...
      const rebuildFromComposerInputStream = Boolean(
        shouldRecreateComposerForEffectRouteChange &&
        composerInputStream &&
      ...
      const mixedStream = await this._mediaPipeline.applyMediaStreamComposerOnSdkGumStream(
        rebuildFromComposerInputStream ? composerInputStream : (() =>
        {
          const currentLocalVideoStream = new MediaStream();
    ...
    const patch = Object.assign({}, resolvedOptions);
  ...
  connect(target, options = {}, initCallback) 
  {
    ...
    const composerOptions = this._mediaPipeline.resolveMediaStreamComposerOptions(options);
    ...
    if (type === 'camera') 
    {
      ...
      return Promise.resolve()
        ...
        .then(async({ constraints: videoConstraints, useComposerBranch }) => 
        {
          ...
          const sessionComposerOptions = this._sessionMediaStreamComposerOptions;
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() => 
      {
        ...
        if (this._remoteToVideo && !this._localToAudio && this._notHold && !hasVideo && (this._customMediaStream === false)) 
        {
          ...
          return this._mediaPipeline.getUserMediaWithSessionPipeline(videoConstraints, this._sessionMediaStreamComposerOptions)
            .catch((error) =>
            {
              if (this._status === C.STATUS_TERMINATED)
              {
