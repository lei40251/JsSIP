...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua)
  {
    ...
    this._transactionId = Math.floor(Math.random() * 99) + 1;
    // BFCP的心跳定时器
    ...
    this.MAX_CHUNK_SIZE = 1048576;
    this.chunkSize = null;
    this.lowWaterMark = null;
    this.highWaterMark = null;
    this.sendPressmax = null;
    this.sendPressvalue = null;
    this.numberOfSendCalls = null;
    this.maxTimeUsedInSend = null;
    this.totalTimeUsedInSend = null;
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
           ...
           * 是否启用 DataChannel
           **/
          this._enableBFCP && (this._connection.ondatachannel =(event) => { this._initDataChannel(event); });
        }
      })
      ...
      return Promise.resolve()
          ...
          this._connection.getSenders().find((s) =>
          {
            ...
            if (s.track.kind == 'video')
            {
              if (this._enableBFCP)
              {
                ...
                (s.track != this._bfcpVideoTrack) && s.track.stop();
              }
              else
              {
                s.track.stop();
        ...
        .then((stream) =>
        {
          ...
          const sender = this._connection.getSenders().find((s) =>
          {
            ...
            {
              // 启用了BFCP，区分一下BFCP控制的视频轨道
              return s.track.kind == 'video' && s.track != this._bfcpVideoTrack;
            }
            else
            {
   ...
   * 分享媒体
   */
  async share(type, id, assembly, dual)
  {
    logger.debug('share()');
    ...
    if ((dual && !this._enableBFCP) || (!dual && this._enableBFCP))
    {
      ...
      //   `Dual and BFCP settings must be consistent. Dual: ${ dual }, BFCP: ${ this._enableBFCP}`
      // );
    }
    ...
    try
    {
      if (this._enableBFCP)
      {
        ...
        const status = floorResponse.getAttribute('FloorRequestInformation').content[1].content[1].content[0];
          ...
          .then(() =>
          {
            this._connection.setRemoteDescription(answer)
              .then(async() =>
              {
                ...
                if (this._replaceAudioTrack && (navigator.userAgent.indexOf('WeChat') != -1 || navigator.userAgent.indexOf('ArkWeb') != -1))
                {
                ...
                if (this._enableBFCP)
                {
                  this._bfcpVideoTrack = Utils.generateAnEmptyVideoTrack();
        ...
        if (dual)
        {
          this._connection.getTransceivers().forEach((transeiver) =>
          {
            if (transeiver.sender.track && transeiver.sender.track.kind === 'video')
            {
              ...
              {
                transeiver.sender.replaceTrack(this._bfcpVideoTrack);
              }
            }
          });
  ...
  {
    logger.info(`BFCP send HACK: Transaction ID: ${message.commonHeader.transactionId}, Timestamp: ${Date.now()}`);
  ...
  {
    logger.info(`BFCP send FloorRequestStatusACK: Transaction ID: ${message.commonHeader.transactionId}, Timestamp: ${Date.now()}`);
  ...
  {
    const floorStatus = message.getAttribute('FloorRequestInformation').content[1].content[1].content[0];
  ...
  _sendDataChannelMessage(message, transactionId)
  {
    if (transactionId)
    {
      logger.info(`Sending message with Transaction ID: ${transactionId}, Timestamp: ${Date.now()}`);
    }
  ...
  _onChannelMessage(event)
  {
    ...
    try
    {
      ...
      logger.info(`BFCP recv: ${JSON.stringify(message)}, Transaction ID: ${message.commonHeader.transactionId}, Timestamp: ${Date.now()}`);
  ...
  _dataChannelSend(message, transactionId)
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      if (!this._dataChannelReady)
      {
        reject(new Error(`[DataChannel] Not ready for transactionId: ${transactionId}`));
      ...
      {
        reject(new Error(`[DataChannel] Max retry attempts (${CRTC_C.MAX_RETRY_ATTEMPTS}) reached for transactionId: ${transactionId}`));

        return;
      }

      // 输出日志：发送消息次数及tid，时间
      logger.debug(`BFCP send: ${this._bfcpUser.receiveMessage(messageState.message)} ${messageState.retries + 1}, ${transactionId} ${Date.now()}`);
  ...
  _initDataChannel(event)
  {
    ...
    datachannel.onopen = () =>
    {
      ...
      this.chunkSize = Math.min(this._connection.sctp.maxMessageSize, this.MAX_CHUNK_SIZE);
      console.warn('Determined chunk size: ', this.chunkSize);
      this.dataString = new Array(this.chunkSize).fill('X')
        .join('');
      ...
      console.warn('Send buffer low water threshold: ', this.lowWaterMark);
      console.warn('Send buffer high water threshold: ', this.highWaterMark);
      this._dataChannel.bufferedAmountLowThreshold = this.lowWaterMark;
      this._dataChannel.addEventListener('bufferedamountlow', (e) =>
      {
      ...
      this.sendProgressmax = 1 * 1024 * 1024;
  ...
  _sendData()
  {
    ...
    let bufferedAmount = this._dataChannel.bufferedAmount;
    ...
    while (this.sendProgressvalue < this.sendProgressmax)
    {
      ...
      if (bufferedAmount >= this.highWaterMark)
      {
        ...
        {
          this.timeoutHandle = setTimeout(() => this._sendData(), 0);
        }
        console.warn(`Paused sending, buffered amount: ${bufferedAmount} (announced: ${this._dataChannel.bufferedAmount})`);
        break;
      }
    }
