...
module.exports = class RTCSession extends EventEmitter 
{
   ...
   */
  _parseUpgradeToVideoAccept(arg)
  {
      ...
      .then((desc) => 
      {
        ...
        request.reply(200, null, extraHeaders,
          ...
          () => 
          {
            ...
            this._setInvite2xxTimer(request, desc, extraHeaders);
            this._setACKTimer();
  ...
  async setVideoContentHint(hint = '', shared = false)
  {
    ...
    // 设置 contentHint
      if ('contentHint' in track)
      {
        ...
        {
          logger.warn(`set contentHint failed: ${err.message}`);
        }
      }
 ...
 */
   ...
   */
  _setInvite2xxTimer(request, body, extraHeaders) 
  {
    let timeout = Timers.T1;

    // 保存首次 200 OK 的完整应答头，重传时保持一致，避免自定义 SIP 头丢失。
    const responseHeaders = Utils.cloneArray(extraHeaders);
        ...
        else if (this._mode === 'audio') 
        {
          ...
          else 
          {
            ...
            this.emit('upgradeToVideo', {
              ...
              accept : (arg) =>
              {
                this._parseUpgradeToVideoAccept(arg);
                this._localToAudio = false;
                nextS.call(this);
              },
              reject : () => 
  ...
  _processInDialogSdpOffer(request) 
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() => 
      {
        ...
        if (this._remoteToVideo && !this._localToAudio && this._notHold && !hasVideo)
        {
          // 优先使用 upgradeToVideo 事件 accept() 传入的自定义视频流/轨道（无摄像头场景）
          if (this._upgradeVideoStream)
          {
            const customStream = this._upgradeVideoStream;
            this._upgradeVideoStream = null;
            this._customMediaStream = true;
          ...
          if (this._customMediaStream === false)
          {
