    ...
    return Promise.resolve()
      ...
      .then(async() =>
      {
        ...
        else if (this._inviteMediaConstraints.audio || this._inviteMediaConstraints.video)
        {
          ...
          const mStream = await navigator.mediaDevices.getUserMedia(currMediaConstraints)
            .catch((error) =>
            {
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        if (this._remoteToVideo && this._notHold && !hasVideo)
        {
    ...
    navigator.mediaDevices.ondevicechange = () =>
    {
      ...
      timer = setTimeout(() =>
      {
        ...
        this._connection.getSenders().forEach((sender) =>
        {
          ...
          if (sender.track && sender.track.kind === 'video')
          {
            this._replaceVideoToCanvas.call(sender.track);
          }
          // 音频轨道
          else if (sender.track && sender.track.kind === 'audio')
          {
            this._replaceMicToAudio.call(sender.track);
          }
        });
      }, 300);
    };
