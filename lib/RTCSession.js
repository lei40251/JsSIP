      ...
      .then((stream) =>
      {
        ...
        stream.getTracks().forEach((track) =>
        {
          this._inviteVideoTrackStatsTimer = setInterval(() =>
          {
            ...
            logger.debug(`${track.kind} track state: ${JSON.stringify(trackObj)} ***** settings: ${JSON.stringify(track.getSettings())} ***** constraints: ${JSON.stringify(track.getConstraints())} ***** capabilities: ${JSON.stringify(track.getCapabilities())}`);
        ...
        this._sendInitialRequest(mediaConstraints, rtcOfferConstraints, stream);
      });
  }
        ...
        if (stream)
        {
          ...
          stream.getTracks().forEach((track) =>
          {
            this._answerVideoTrackStatsTimer = setInterval(() =>
            {
              ...
              logger.debug(`${track.kind} settings: ${JSON.stringify(track.getSettings())}`);
              logger.debug(`${track.kind} constraints: ${JSON.stringify(track.getConstraints())}`);
              logger.debug(`${track.kind} capabilities: ${JSON.stringify(track.getCapabilities())}`);

              logger.debug(`${track.kind} track state: ${JSON.stringify(trackObj)}`);
