...
module.exports = class RTCSession extends EventEmitter
{
  ...
  _receiveInviteResponse(response)
  {
    ...
    switch (true)
    {
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        // 获取响应中BFCP相关属性
        this._floorId = (e.sdp.match(/a=floorid:(\d+)/) || [ null, 1 ])[1];
  ...
  _onChannelMessage(event)
  {
    ...
    try
    {
      ...
      console.warn(`BFCP recv: ${JSON.stringify(message)}, Transaction ID: ${message.commonHeader.transactionId}, Timestamp: ${Date.now()}`);
  ...
  _dataChannelSend(message, transactionId)
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      // 输出日志：发送消息次数及tid，时间戳
      logger.debug(`BFCP send: ${JSON.stringify(this._bfcpUser.receiveMessage(messageState.message))} ${messageState.retries + 1}, ${transactionId} ${Date.now()}`);
