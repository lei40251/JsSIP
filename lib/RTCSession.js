...
const MediaStreamComposer = require('./MediaStreamComposer');
const AiNSEngine = require('./AINoiseSuppression/index.js');
...
module.exports = class RTCSession extends EventEmitter 
{
  ...
  async _processMediaStream(_stream)
  {
    ...
    try
    {
      const result = await this._mediaStreamProcessor(_stream);
  ...
  {
    const engine = this._sessionAiNSEngine;
    const aiNSInputStream = this._aiNSInputStream;
  ...
  async _applyAiNoiseSuppressionOnSdkGumStream(stream, aiNSOptions)
  {
    ...
    const normalizedOptions = this._normalizeSessionAiNSOptions(aiNSOptions);
    ...
    try
    {
      ...
      const processedStream = await this._sessionAiNSEngine.process(stream);
  ...
  async _replaceAudioTrackWithSessionAiNoiseSuppression(stream, aiNSOptions = this._sessionAiNSOptions)
  {
    ...
    try
    {
      ...
      const processedStream = await this._sessionAiNSEngine.replaceAudioTrack(stream);
  ...
  _getGumConstraintsWithProcessorFlags(constraints, aiNSOptions = null)
  {
    ...
    if (
      (
        this._normalizeSessionAiNSOptions(aiNSOptions) ||
        (this._mediaStreamProcessor && this._mediaStreamProcessor.disableNativeNoiseSuppression)
      ) &&
  ...
  _stopSessionMixer()
  {
    const mixerInputStream = this._mixerInputStream;
  ...
  async _applyMixerOnSdkGumStream(stream, mixerOptions)
  {
    ...
    const mixerCtorOptions = this._buildMediaStreamComposerCtorOptions(stream, mixerOptions);
    let mixer = null;
    ...
    try
    {
      ...
      const mixedVideoStream = mixer.getVideoStream();
  ...
  async _getUserMediaWithSessionPipeline(constraints, mixerOptions, aiNSOptions = this._sessionAiNSOptions)
  {
    ...
    const gumConstraints = this._getGumConstraintsWithProcessorFlags(constraints, aiNSOptions);
    const stream = await navigator.mediaDevices.getUserMedia(gumConstraints);
    const processedStream = await this._processMediaStream(stream);
    const aiNoiseSuppressedStream = await this._applyAiNoiseSuppressionOnSdkGumStream(processedStream, aiNSOptions);
  ...
  connect(target, options = {}, initCallback) 
  {
    ...
    const mixerOptions = this._resolveMediaStreamComposerOptions(options);
    ...
    return Promise.resolve()
      ...
      .then(async() => 
      {
        ...
        if (this._inviteMediaConstraints.audio || this._inviteMediaConstraints.video) 
        {
          ...
          {
            const tStream = await this._getUserMediaWithSessionPipeline(currMediaConstraints, mixerOptions)
              .catch((error) =>
              {
    ...
    Promise.resolve()
      ...
      .then(async() => 
      {
        ...
        if (mediaConstraints.audio || mediaConstraints.video) 
        {
          ...
          const mStream = await this._getUserMediaWithSessionPipeline(mediaConstraints, mixerOptions)
            .catch((error) =>
            {
              if (this._status === C.STATUS_TERMINATED)
              {
  ...
  upgradeToVideo(options, done) 
  {
    ...
    if (
      Object.prototype.hasOwnProperty.call(options, 'mediaStreamComposer') ||
      Object.prototype.hasOwnProperty.call(options, 'mixer')
    )
    {
    ...
    return Promise.resolve()
      .then(async() => 
      {
        const mixerOptions = this._sessionMixerOptions;
        ...
        {
          stream = await this._getUserMediaWithSessionPipeline({ video: videoConstraints }, mixerOptions)
    ...
    if (type === 'camera') 
    {
      ...
      return Promise.resolve()
        .then(() => 
        {
          ...
          const useMixerBranch = Boolean(mixerOptions && this._mixer && this._mixerInputStream);
          ...
          return { constraints, useMixerBranch };
        })
        .then(async({ constraints: videoConstraints, useMixerBranch }) => 
        {
          ...
          try
          {
            const currentMixer = this._mixer;
            const oldInputStream = this._mixerInputStream;
            ...
            const mixedVideoStream = currentMixer.getVideoStream();
      ...
      return Promise.resolve()
        .then(() => 
        {
          ...
          return navigator.mediaDevices
            .getUserMedia(this._getGumConstraintsWithProcessorFlags(constraints))
            .catch((error) => 
            {
              this._logEventError('error', 'getusermediafailed', error);
              this.emit('getusermediafailed', error);
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() => 
      {
        ...
        if (this._remoteToVideo && !this._localToAudio && this._notHold && !hasVideo && (this._customMediaStream === false)) 
        {
          ...
          return this._getUserMediaWithSessionPipeline(videoConstraints, this._sessionMixerOptions)
            .catch((error) =>
            {
              if (this._status === C.STATUS_TERMINATED)
              {
  ...
  _replaceCanvasToVideo() 
  {
    ...
    this._getUserMediaWithSessionPipeline({ audio: false, video: this._inviteMediaConstraints.video || true }, this._sessionMixerOptions)
      .then((stream) => 
      {
        this._connection.getSenders().forEach((sender) => 
        {
