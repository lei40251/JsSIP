...
module.exports = class RTCSession extends EventEmitter
{
  ...
  switchVideoStream(videoStream)
  {
    ...
    {
      that._connection.getSenders().find(function(s)
      {
      ...
      .then(() =>
      {
        ...
        if (type === 'offer')
        {
          ...
          return navigator.mediaDevices.getUserMedia({
            audio:constraints &&
            constraints.offerToReceiveAudio ? constraints.offerToReceiveAudio : true,
            video:constraints &&
            constraints.offerToReceiveVideo ? constraints.offerToReceiveVideo : true
          }).catch((error) =>
          {
            if (this._status === C.STATUS_TERMINATED)
            {
        ...
        return new Promise((resolve) =>
        {
          ...
          connection.addEventListener('icecandidate', iceCandidateListener = (event) =>
          {
            ...
            setTimeout(function()
            {
              ...
              resolve(e.sdp);
            }, this._ua.configuration.ice_checking_timeout);
