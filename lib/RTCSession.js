...
module.exports = class RTCSession extends EventEmitter
{
  ...
  async share(type, id, assembly, dual)
  {
    ...
    {
      throw new Exceptions.NotSupportedError(`Dual and BFCP settings must be consistent. Dual: ${ dual }, BFCP: ${ this._enableBFCP}`);
    }
  ...
  _handleFloorStatusMessage(message)
  {
    ...
    {
      this.emit('remoteShared', { streamIndex: this._transceiverIndex });
    }
    else if (floorStatus === RequestStatusValue.Released)
    {
      this.emit('remoteShared', { streamIndex: false });
    }
    else
    {
