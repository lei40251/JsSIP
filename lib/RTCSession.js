...
const WebRTC_SwitchCam = require('./WebRTC/SwitchCam');
const URI = require('./URI');
const debug = require('debug')('FlyInn:RTCSession');
const debugerror = require('debug')('FlyInn:ERROR:RTCSession');
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua)
  {
    ...
    // Display Shared 分享桌面
    this._displayRTPSender = null;
    this._displayMediaStream = null;
    ...
    // 切换视频流
    if (this._ua.configuration.switch_camera)
    {
      this._switchCam = new WebRTC_SwitchCam(this);
    }
    else
    {
      this._switchCam = null;
    }
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    const preferredAudioCodec = options.preferredAudioCodec || null;
    const preferredVideoCodec = options.preferredVideoCodec || null;
    const inviteWithoutSdp = options.inviteWithoutSdp || false;
    ...
    if (inviteWithoutSdp)
    {
      ...
      Promise.resolve()
        .then(() =>
        {
          ...
          log.debug(`emit "sending" ${this._request}`);
    ...
    else
    {
      this._sendInitialRequest(
        mediaConstraints, rtcOfferConstraints, mediaStream, preferredAudioCodec,
        preferredVideoCodec
      );
    }
  }
  ...
  init_incoming(request, initCallback)
  {
    ...
    // 呼入通话时触发 UA 的 callmode 事件
    this._callMode('remote', request);
    ...
    Promise.resolve()
        ...
        else if (mediaConstraints.audio || mediaConstraints.video)
        {
          ...
          return navigator.mediaDevices.getUserMedia(mediaConstraints)
            .catch((error) =>
            {
              ...
              log.error(`emit "getusermediafailed" [error:${error}]`);
      ...
      .then(() =>
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .catch((error) =>
          {
            ...
            log.error(`emit "peerconnection:setremotedescriptionfailed" [error:${error}]`);
      ...
      .then((desc) =>
      {
        ...
        if (this._callmode !== callmode)
        {
          ...
          debug('emit "callmode"');
          log.debug('emit "callmode"');
          this.emit('callmode', { originator: 'local', mode: this._callmode });
        }
  ...
  sendDTMF(tones, options = {})
  {
    ...
    log.debug(`sendDTMF() | tones: ${tones}`);
    ...
    else if (duration < RTCSession_DTMF.C.MIN_DURATION)
    {
      ...
      log.debug(`"duration" value is lower than the minimum allowed, setting it to ${RTCSession_DTMF.C.MIN_DURATION} milliseconds`);
    ...
    else if (duration > RTCSession_DTMF.C.MAX_DURATION)
    {
      ...
      log.debug(`"duration" value is greater than the maximum allowed, setting it to ${RTCSession_DTMF.C.MAX_DURATION} milliseconds`);
    ...
    else if (interToneGap < RTCSession_DTMF.C.MIN_INTER_TONE_GAP)
    {
      ...
      log.debug(`"interToneGap" value is lower than the minimum allowed, setting it to ${RTCSession_DTMF.C.MIN_INTER_TONE_GAP} milliseconds`);
   ...
   */
  switchCam(videoConstraints)
  {
    debug('switchVideoStream()');
    log.debug('switchVideoStream()');
    ...
    return this._switchCam.switchVideoStream(null, videoConstraints)
      .then((stream) =>
      {
  ...
  */
  displayShare(type)
  {
    if (!(navigator.mediaDevices && 'getDisplayMedia' in navigator.mediaDevices))
    {
      ...
      log.error('getDisplayMedia is not supported');
    ...
    navigator.mediaDevices.getDisplayMedia({
      ...
      .then((stream) =>
      {
        ...
        this._displayMediaStream = stream;
        ...
        else
        {
          stream.getVideoTracks().forEach((track) =>
          {
            this._displayRTPSender = this._connection.addTrack(track, stream);
            // this._connection.addTransceiver(stream.getVideoTracks()[0], { direction: 'sendonly' });
      ...
      .catch((error) =>
      {
        ...
        log.error(`emit "getdisplaymediafailed" [error:${error}]`);
  ...
  unDisplayShare(type)
  {
    ...
    log.debug('unDisplayShare() | closing display MediaStream');
  ...
  toAudio(options = {}, done)
  {
    ...
    this.emit('callmode', { originator: 'local', mode: 'audio' });
  ...
  // 切换到视频模式
  toVideo(options = { video: true, audio: false }, done)
  {
    debug('toVideo()');
    log.debug('toVideo()');
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        this.emit('callmode', { originator: 'local', mode: 'video' });
  ...
  _isReadyToReOffer()
  {
    if (! this._rtcReady)
    {
      ...
      log.debug('_isReadyToReOffer() | internal WebRTC status not ready');
    ...
    if (! this._dialog)
    {
      ...
      log.debug('_isReadyToReOffer() | session not established yet');
        ...
        this._dialog.uas_pending_reply === true)
    {
      ...
      log.debug('_isReadyToReOffer() | there is another INVITE/UPDATE transaction in progress');
  ...
  _close()
  {
    ...
    if (this._connection)
    {
      ...
      catch (error)
      {
        ...
        log.error(`close() | error closing the RTCPeerConnection: ${error}`);
      }
    }
    ...
    if (this._localMediaStream && this._localMediaStreamLocallyGenerated)
    {
      ...
      log.debug('close() | closing local MediaStream');
    ...
    if (this._displayMediaStream && this._displayMediaStreamLocallyGenerated)
    {
      ...
      log.debug('close() | closing display MediaStream');
  ...
  _setACKTimer()
  {
    this._timers.ackTimer = setTimeout(() =>
    {
      if (this._status === C.STATUS_WAITING_FOR_ACK)
      {
        ...
        log.debug('no ACK received, terminating the session');
  ...
  _createLocalDescription(type, constraints)
  {
    ...
    log.debug('createLocalDescription()');
        ...
        if (type === 'offer')
        {
          return connection.createOffer(constraints)
            .catch((error) =>
            {
              ...
              log.error(`emit "peerconnection:createofferfailed" [error:${error}]`);
        ...
        else
        {
          return connection.createAnswer(constraints)
            .catch((error) =>
            {
              ...
              log.error(`emit "peerconnection:createanswerfailed" [error:${error}]`);
      ...
      .then((desc) =>
      {
        return connection.setLocalDescription(desc)
          .catch((error) =>
          {
            ...
            log.error(`emit "peerconnection:setlocaldescriptionfailed" [error:${error}]`);
      ...
      .then(() =>
      {
        ...
        return new Promise((resolve) =>
        {
          ...
          connection.addEventListener('icecandidate', iceCandidateListener = (event) =>
          {
            ...
            // 添加 ice 收集超时控制
            let iceGatheringDuration = (this._ua.configuration.ice_gathering_timeout*100);

            if (iceGatherFlag)
            {
              const iceGatheringTimer = setInterval(() =>
              {
                iceGatheringDuration -= 10;
  ...
  _receiveReinvite(request)
  {
    ...
    if (callmode === 'video' && this._callmode === 'audio')
    {
      ...
      {
        this.emit('inviteToVideo', (acceptVideo) =>
        {
          nextS.call(this, acceptVideo);
        });
      }
    }
    else
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    log.debug('_processInDialogSdpOffer()');

    let videoConstraints = { video: true };

    const sdp = request.parseSDP();
    const acceptVideo = request.acceptVideo;
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      .then(() =>
      {
        if (this._callmode !== callmode && callmode === 'video')
        {
          this._localMediaStreamLocallyGenerated = true;
  ...
  {
    debug('receiveRefer()');
    log.debug('receiveRefer()');

    if (!request.refer_to)
    {
      debug('no Refer-To header field present in REFER');
      log.debug('no Refer-To header field present in REFER');
      request.reply(400);
    ...
    if (request.refer_to.uri.scheme !== JsSIP_C.SIP)
    {
      ...
      log.debug('Refer-To header field points to a non-SIP URI scheme');
      request.reply(416);
  ...
  _receiveReplaces(request)
  {
    ...
    function reject()
    {
      ...
      log.debug('Replaced INVITE rejected by the user');
      request.reply(486);
    }
  ...
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream,
    preferredAudioCodec, preferredVideoCodec)
  {
    const request_sender = new RequestSender(this._ua, this._request, {
      onRequestTimeout : () =>
      {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        {
          const [ mimeType, sdpFmtpLine ] = preferredAudioCodec.split(' ');
          ...
          const transceiver = this._connection.getSenders().find((sender) =>
          {
        ...
        {
          const [ mimeType, sdpFmtpLine ] = preferredVideoCodec.split(' ');
      ...
      .then((desc) =>
      {
        ...
        debug('emit "sending" [request:%o]', this._request);
        log.debug(`emit "sending" [request:${this._request}]`);
  ...
  _getDTMFRTPSender()
  {
    ...
    if (!(sender && sender.dtmf))
    {
      ...
      log.error('sendDTMF() | no local audio track to send DTMF with');
  ...
  _receiveInviteResponse(response)
  {
    ...
    log.debug('receiveInviteResponse()');
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        if (!response.to_tag)
        {
          ...
          log.debug('1xx response received without to tag');
          break;
        }
  ...
  _mangleOffer(sdp)
  {
    ...
    if (this._localHold && ! this._remoteHold)
    {
      ...
      log.debug('mangleOffer() | me on hold, mangling offer');
      for (const m of sdp.media)
      {
        if (holdMediaTypes.indexOf(m.type) === -1)
        {
    ...
    else if (this._localHold && this._remoteHold)
    {
      ...
      log.debug('mangleOffer() | both on hold, mangling offer');
      for (const m of sdp.media)
      {
        if (holdMediaTypes.indexOf(m.type) === -1)
        {
    ...
    else if (this._remoteHold)
    {
      ...
      log.debug('mangleOffer() | remote on hold, mangling offer');
      for (const m of sdp.media)
      {
        if (holdMediaTypes.indexOf(m.type) === -1)
        {
  ...
  _runSessionTimer()
  {
    ...
    if (this._sessionTimers.refresher)
    {
      this._sessionTimers.timer = setTimeout(() =>
      {
        ...
        log.debug('runSessionTimer() | sending session refresh request');
    ...
    else
    {
      this._sessionTimers.timer = setTimeout(() =>
      {
        ...
        log.error('runSessionTimer() | timer expired, terminating the session');
  ...
  _callMode(originator, request)
  {
    ...
    if (this._callmode !== callmode)
    {
      ...
      this._ua.callMode(this, { originator: originator, mode: this._callmode });
    }
  }
  ...
  _progress(originator, response)
  {
    ...
    log.debug('session progress');

    debug('emit "progress"');
    log.debug('emit "progress"');
  ...
  _onunhold(originator)
  {
    ...
    log.debug('session onunhold');

    this._setLocalMediaStatus();

    debug('emit "unhold"');
    log.debug('emit "unhold"');
  ...
  _onmute({ audio, video })
  {
    ...
    log.debug('session onmute');

    this._setLocalMediaStatus();

    debug('emit "muted"');
    log.debug('emit "muted"');
  ...
  _onunmute({ audio, video })
  {
    ...
    log.debug('session onunmute');

    this._setLocalMediaStatus();

    debug('emit "unmuted"');
    log.debug('emit "unmuted"');
