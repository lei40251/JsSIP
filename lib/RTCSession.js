...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua)
  {
    ...
    // 动态更新流用
    this._bfcpUserMediaStrem = new MediaStream();
  ...
  sendFloorStatus()
  {
    ...
    const floorStatus = this._bfcpUser.floorStatusMessage(this._floorId, 3, currentTransactionId);
  ...
  _sendDataChannelMessage(message, transactionId)
  {
    if (transactionId)
    {
      logger.debug(`Sending message with Transaction ID: ${transactionId}, Timestamp: ${Date.now()}`);
    }
