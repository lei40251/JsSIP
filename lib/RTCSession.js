    ...
    return Promise.resolve()
      ...
      .then((desc) =>
      {
        ...
        if (type === 'offer')
        {
          ...
          if (sdp.media[1])
          {
            sdp.media[1].payloads = '102';
            sdp.media[1].fmtp && (sdp.media[1].fmtp = sdp.media[1].fmtp.filter((r) => r.payload == '102'));
            sdp.media[1].rtp && (sdp.media[1].rtp = sdp.media[1].rtp.filter((r) => r.payload == '102'));
            sdp.media[1].rtcpFb && (sdp.media[1].rtcpFb = sdp.media[1].rtcpFb.filter((r) => r.payload == '102'));
  
            desc.sdp = sdp_transform.write(sdp);
  
            desc.sdp = desc.sdp.replace(/a=rtcp-fb:102 goog-remb\r\n/, 'a=rtcp-fb:102 ccm tmmbr\r\n');
