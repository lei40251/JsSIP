...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua)
  {
    ...
    document.addEventListener('visibilitychange', () =>
    {
      ...
      if (document.hidden)
      {
        if (this._connection.connectionState==='connected' && this._is_confirmed && !this._enableBFCP)
        {
          this._connection.getSenders().forEach((sender) =>
          {
            if (sender.track.kind === 'video')
            {
              this._visibilitychangeVideoTrack = sender.track;

              console.warn('track: ', sender.track);

              // eslint-disable-next-line max-len
              this._blackVideoTrack = Utils.generateAnBlackVideoTrack({ hidden: true, width: sender.track.getSettings().width, height: sender.track.getSettings().height });

              sender.replaceTrack(this._blackVideoTrack.videoTrack);
            }
          });
        }
      }
      // console.warn('页面回到前台');
      else if (this._blackVideoTrack)
      {
        this._connection.connectionState==='connected' && this._connection.getSenders().forEach((sender) =>
        {
          if (sender.track.kind === 'video')
          {
            sender.replaceTrack(this._visibilitychangeVideoTrack);
            this._visibilitychangeVideoTrack = null;
  ...
  _createRTCConnection(pcConfig, rtcConstraints)
  {
        ...
        case 'connecting':
          ...
          if (!successfullyConnected)
          {
            setTimeout(() =>
            {
              ...
              {
                logger.warn(`start iceConnectionState ${self._connection.connectionState}`);
    ...
    this._connection.addEventListener('iceconnectionstatechange', () =>
    {
      ...
      if (state === 'failed' || state === 'disconnected')
      {
        ...
        {
          logger.warn(`iceConnectionState ${state}`);
    ...
    return Promise.resolve()
      ...
      .then(() =>
      {
          ...
          (connection.iceGatheringState === 'gathering' && this._iceReady))
        {
          ...
          logger.debug(`complete emit "sdp"${ e.sdp}`);
          this.emit('sdp', e);
    ...
    if (request.body)
    {
      ...
      for (const m of sdp_request.media)
      {
        ...
        else if (this._mode === 'audio')
        {
          ...
          {
            logger.debug('has upgradeToVideo listeners.');
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        logger.debug('stats: ', this._remoteToVideo, this._localToAudio, this._notHold, hasVideo, this._customMediaStream);
  ...
  _ontogglemode(mode)
  {
    logger.debug(`ontogglemode() ${mode} ${this._mode}`);
  ...
  _onChannelMessage(event)
  {
    ...
    try
    {
      ...
      {
        // 记录BFCP消息响应时间
        logger.debug(`bfcp response time: transactionId(${message.commonHeader.transactionId}), time(${Date.now() - this._dataChannelMsgs[message.commonHeader.transactionId].sendAt}ms)`);
      ...
      switch (message.commonHeader.primitive)
      {
        ...
        default:
          logger.debug(`onChannelMessage(): do not require processing type: ${message.commonHeader.primitive}`);
          break;
      }
    }
    catch (error)
  ...
  _dataChannelSend(message, transactionId)
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      {
        logger.warn(`[DataChannel] Max retry attempts (${CRTC_C.MAX_RETRY_ATTEMPTS}) reached for transactionId: ${transactionId}`);
  ...
  {
    logger.debug(`initDataChannel()${ JSON.stringify(event)}`);
