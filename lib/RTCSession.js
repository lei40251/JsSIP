...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua)
  {
    ...
    this._bfcpUser = new BFCPUser(Math.floor(Math.random() * 9999) + 1, Math.floor(Math.random() * 999999) + 1);
    this._floorId = 2; // TODO SDP协商获得值
    this._floorctrl = 'c-s';
    this._mStream = null; // 本地流的 mid
    this._mstrm = null; // 远端流的 label
    this._transceiverIndex= null;
    this._bfcpUserId = null;
    this._floorRequestId = null;
    this._confId = null; // SDP协商获得值
    this._transactionId = Math.floor(Math.random() * 9999) + 1; // 发送BFCP消息事务ID，起始值为1-9的随机整数
    ...
    else if (type === 'screen')
    {
      ...
      return this._sendFloorRequest()
        .then((response) =>
        {
          ...
          {
            this._floorRequestId = response.getAttribute('FloorRequestInformation').content[0];

            return Promise.reject(`not accept: ${response.getAttribute('FloorRequestInformation').content[1].content[1].content[0]}`);
          }
        })
        .then(() =>
        {
  ...
  _onChannelMessage(event)
  {
    ...
    try
    {
      const message = this._bfcpUser.receiveMessage(data);
      let response;
      ...
      switch (message.commonHeader.primitive)
      {
        ...
        case Primitive.FloorRequest:
        {
          ...
          {
            response = this._bfcpUser.floorRequestStatusMessage(
              message,
              wantedFloorId,
  ...
  _dataChannelSend(message, transactionId)
  {
    return new Promise((resolve, reject) =>
    {
      ...
      {
        // TODO
        console.warn(`Max retry attempts reached for messageId: ${transactionId}`);
        reject(new Error(`Max retry attempts reached for messageId: ${transactionId}`));

        return;
      }

      // TODO 打印发送消息次数及tid，时间
      console.warn(`BFCP send: ${this._bfcpUser.receiveMessage(messageState.message)} ${messageState.retries + 1}, ${transactionId} ${Date.now()}`);
