...
module.exports = class RTCSession extends EventEmitter
{
  ...
  _sendFloorRequest()
  {
    const floorRequest = this._bfcpUser.floorRequestMessage(this._transactionId, this._floorId);
  ...
  _sendHello()
  {
    const hello = this._bfcpUser.helloMessage(this._transactionId, this._floorId);
  ...
  _sendFloorRelease()
  {
    const floorRelease = this._bfcpUser.floorReleaseMessage(this._transactionId, this._floorRequestId);
  ...
  _dataChannelSend(message, transactionId)
  {
    ...
    // 发送消息
    console.warn(`BFCP send: ${messageState.retries + 1}, ${transactionId} ${Date.now()}`);
  ...
  _initDataChannel(event)
  {
    ...
    { // No "onclosing" event
      if (this._dataChannel && this._dataChannel.readyState === 'closing')
      {
