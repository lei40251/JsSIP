...
module.exports = class RTCSession extends EventEmitter 
{
   ...
   *   getUserMedia → _processMediaStream → _applyAiNoiseSuppressionOnSdkGumStream → _applyMediaStreamComposerOnSdkGumStream
   *
   * 流程：
   *   1. 校验输入（非空、有音频轨）
   *   2. 停止旧的降噪引擎实例（避免残留）
  ...
  {
    const composerInputStream = this._mediaStreamComposerInputStream;
  ...
  async _applyMediaStreamComposerOnSdkGumStream(stream, composerOptions)
  {
    ...
    const composerCtorOptions = this._buildMediaStreamComposerCtorOptions(stream, composerOptions);
    let composer = null;
  ...
  connect(target, options = {}, initCallback) 
  {
    ...
    const composerOptions = this._resolveMediaStreamComposerOptions(options);
    ...
    return Promise.resolve()
      ...
      .then(async() => 
      {
        ...
        if (this._inviteMediaConstraints.audio || this._inviteMediaConstraints.video) 
        {
          ...
          {
            const tStream = await this._getUserMediaWithSessionPipeline(currMediaConstraints, composerOptions)
              .catch((error) =>
              {
    ...
    Promise.resolve()
      ...
      .then(async() => 
      {
        ...
        if (mediaConstraints.audio || mediaConstraints.video) 
        {
          ...
          const mStream = await this._getUserMediaWithSessionPipeline(mediaConstraints, composerOptions)
            .catch((error) =>
            {
              if (this._status === C.STATUS_TERMINATED)
              {
  ...
  upgradeToVideo(options, done) 
  {
    ...
    return Promise.resolve()
      .then(async() => 
      {
        const composerOptions = this._sessionMediaStreamComposerOptions;
        ...
        {
          stream = await this._getUserMediaWithSessionPipeline({ video: videoConstraints }, composerOptions)
      ...
      return Promise.resolve()
        .then(() => 
        {
          ...
          const useComposerBranch = Boolean(
            composerOptions &&
            this._mediaStreamComposer &&
            this._mediaStreamComposerInputStream
          );
        ...
        .then(async({ constraints: videoConstraints, useComposerBranch }) => 
        {
          ...
          try
          {
            const currentComposer = this._mediaStreamComposer;
            const oldInputStream = this._mediaStreamComposerInputStream;
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() => 
      {
        ...
        if (this._remoteToVideo && !this._localToAudio && this._notHold && !hasVideo && (this._customMediaStream === false)) 
        {
          ...
          return this._getUserMediaWithSessionPipeline(videoConstraints, this._sessionMediaStreamComposerOptions)
            .catch((error) =>
            {
              if (this._status === C.STATUS_TERMINATED)
              {
  ...
  _replaceCanvasToVideo() 
  {
    ...
    this._getUserMediaWithSessionPipeline(
      ...
      this._sessionMediaStreamComposerOptions
    )
      .then((stream) => 
      {
        this._connection.getSenders().forEach((sender) => 
