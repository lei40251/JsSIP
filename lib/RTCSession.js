...
const RTCSession_Notify = require('./RTCSession/Notify');
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  get local_stream()
  {
    ...
    const peerConnection = this._connection;
      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            const e = { originator: 'remote', type: 'answer', sdp: request.body, display_mode: this._viewMode(request.body) };
    ...
    return Promise.resolve()
      ...
      .then(() =>
      {
        ...
        if (connection.iceGatheringState === 'complete')
        {
          ...
          const e = { originator: 'local', type: type, sdp: connection.localDescription.sdp, display_mode: this._viewMode(connection.localDescription.sdp) };
  ...
  _receiveReinvite(request)
  {
    ...
    let rejected = false;
    let reqHasVideo = false;
    let senderHasVideo = false;
    ...
    if (reqHasVideo && !senderHasVideo)
    {
      ...
      {
        this.emit('inviteToVideo', (rejectVideo) => {
          nextS.call(this, rejectVideo);
        });
      }
    }
    else
    ...
    function sendAnswer(desc)
    {
      ...
      const e = { originator: 'local', sdp: desc, display_mode: this._viewMode(desc) };
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    const rejectVideo = request.rejectVideo;
    ...
    for (const m of sdp.media)
    {
      ...
      const ip = (m.connection && m.connection.ip) ||
      (sdp.connection && sdp.connection.ip) || false;
    ...
    {
      // this._toggleMuteVideo(true);
      const videoSenders = this._connection.getSenders().filter((sender) =>
      {
  ...
  _receiveInviteResponse(response)
  {
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        const e = { originator: 'remote', type: 'answer', sdp: response.body, display_mode: this._viewMode(response.body) };
  ...
  _sendReinvite(options = {})
  {
    ...
    const rtcOfferConstraints = options.rtcOfferConstraints ||
      this._rtcOfferConstraints || null;
    const toAudio = options.toAudio || false;
  ...
  _viewMode(sdp)
  {
    const sdp_m = sdp_transform.parse(sdp);
