...
module.exports = class RTCSession extends EventEmitter 
{
  ...
  connect(target, options = {}, initCallback) 
  {
    ...
    if (this._ua.sk[7] >= 3) 
    {
      extraHeaders.push(`Contact: ${this._contact};+sip.app-subtype="webrtc-datachannel"`);
 ...
 */
      ...
      .then((sdp) => 
      {
        ...
        sdp_desc.media.forEach((media) => 
        {
          ...
          else
          {
            ...
            media.bandwidth = [ { type: 'AS', limit: 500 } ];
  ...
  _receiveReinvite(request) 
  {
    ...
    function sendAnswer(desc) 
    {
      ...
      if (this._ua.sk[7] >= 3) 
      {
        extraHeaders = [ `Contact: ${this._contact};+sip.app-subtype="webrtc-datachannel"` ];
        extraHeaders.push('Accept-Contact: *;+g.3gpp.icsi-ref="urn%3Aurn-7%3A3gpp-service.ims.icsi.mmtel";video;+sip.app-subtype="webrtc-datachannel"');
        extraHeaders.push('P-Preferred-Service: urn:urn-7:3gpp-service.ims.icsi.mmtel');
      }
      else
  ...
  _receiveUpdate(request) 
  {
    ...
    function sendAnswer(desc) 
    {
      let extraHeaders = [ `Contact: ${this._contact}` ];
  ...
  _sendDataChannelMessage(message, transactionId) 
  {
    ...
    try 
    {
      ...
      console.warn('send DC message:', message);
      this.emit('dc_send', message);      
    }
    catch (error) 
    {
      logger.error(error);
