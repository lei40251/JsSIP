...
module.exports = class RTCSession extends EventEmitter
{
          ...
          if (currMediaConstraints.audio || currMediaConstraints.video)
          {
            const tStream = await navigator.mediaDevices.getUserMedia(currMediaConstraints)
              .catch((error) =>
              {
                ...
                logger.warn(`${this._id} emit "getusermediafailed" [error:%o]${JSON.stringify(error)}`);
            ...
            if (track.kind === 'video')
            {
              if (videoTrackStates.has(track.id))
              {
                ...
                if (track.enabled != trackStat.enabled)
                {
                  ...
                  logger.debug(`${this._id} videoTrackState enabled: ${track.enabled}`);
                }
                ...
                if (track.readyState != trackStat.readyState)
                {
                  ...
                  logger.debug(`${this._id} videoTrackState readyState: ${track.readyState}`);
                }
                ...
                if (track.muted != trackStat.muted || track.mute == true)
                {
                  ...
                  logger.debug(`${this._id} videoTrackState muted: ${track.muted}`);
                }
                ...
                if (track.label != trackStat.label)
                {
                  ...
                  logger.debug(`${this._id} videoTrackState label: ${track.label}`);
                }
              }
              else
              {
  ...
  answer(options = {})
  {
    ...
    Promise.resolve()
      ...
      .then(() =>
      {
        ...
        const e = { originator: 'remote', type: 'offer', sdp: Utils.updateSdpByConstraints(newSdp, mediaConstraints) };
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .catch((error) =>
          {
            ...
            logger.warn(`${this._id} emit "peerconnection:setremotedescriptionfailed" [error:%o]${JSON.stringify(error)}`);
  ...
  upgradeToVideo(options, done)
  {
    ...
    return Promise.resolve()
      .then(async() =>
      {
        ...
        {
          stream = await navigator.mediaDevices.getUserMedia({ video: videoConstraints }).catch((error) => { throw error; });
          if (stream)
          {
            this._environment = await Utils.getHuaweiAndroidEnvironment();
            logger.debug(`${this._id} environment id: `, this._environment);
      ...
      .then(() =>
      {
        ...
        if (options.sendOnly)
        {
          opts = { rtcOfferConstraints: { iceRestart: true, offerToReceiveVideo: false } };
        }

        // 单向视频，仅接收
        if (options.recvOnly)
        {
          opts = { rtcOfferConstraints: { iceRestart: true, offerToReceiveVideo: true } };
        }
        ...
        .then(async(videoConstraints) =>
        {
          ...
          let stream = await navigator.mediaDevices
            ...
            .catch((error) =>
            {
              ...
              logger.error(`${this._id} emit "getusermediafailed" [error:%o]${JSON.stringify(error)}`);
              this.emit('getusermediafailed', error);
              throw new Error('getUserMedia() failed');
            });
    ...
    else if (type === 'screen')
    {
      ...
      return navigator.mediaDevices.getDisplayMedia({ video: { width: { max: 1920 }, height: { max: 1080 }, frameRate: 15 }, audio: true })
        ...
        .catch((error) =>
        {
          ...
          logger.warn(`${this._id} emit "getdisplaymediafailed" [error:%o]${JSON.stringify(error)}`);
  ...
  renegotiate(options = {}, done)
  {
    ...
    const eventHandlers = {
      ...
      failed : () =>
      {
        options.failed && this._failed('remote', 'Media Renegotiation Failed', options.failed);
  ...
  _close()
  {
    ...
    if (this._connection)
    {
      ...
      catch (error)
      {
        ...
        logger.warn(`${this._id} close() | error closing the RTCPeerConnection: %o${JSON.stringify(error)}`);
      }
    }
  ...
  {
    logger.debug(`${this._id} createLocalDescription()`);
        ...
        if (type === 'offer')
        {
          return connection.createOffer(constraints)
            .catch((error) =>
            {
              ...
              logger.warn(`${this._id} emit "peerconnection:createofferfailed" [error:%o]${JSON.stringify(error)}`);
        ...
        else
        {
          return connection.createAnswer(constraints)
            .catch((error) =>
            {
              ...
              logger.warn(`${this._id} emit "peerconnection:createanswerfailed" [error:%o]${JSON.stringify(error)}`);
      ...
      .then((desc) =>
      {
        ...
        return connection.setLocalDescription(desc)
          .catch((error) =>
          {
            ...
            logger.warn(`${this._id} emit "peerconnection:setlocaldescriptionfailed" [error:%o]${JSON.stringify(error)}`);
  ...
  _receiveReinvite(request)
  {
    ...
    if (!request.body)
    {
      ...
      this._connectionPromiseQueue = this._connectionPromiseQueue
        ...
        .catch((e) =>
        {
          logger.warn(this._id + JSON.stringify(e));
          request.reply(500);
        });
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then((stream) =>
      {
        if (stream)
        {
          ...
          stream.getVideoTracks().forEach((track) =>
          {
            ...
            {
              logger.warn(`${this._id} _processInDialogSdpOffer() failed local stream ${error.name} ${track.kind} [error: %o]${JSON.stringify(error)}`);
            }
            ...
            if (RTCPeerConnection.prototype.addTrack)
            {
              ...
              {
                logger.warn(`${this._id} _processInDialogSdpOffer() failed no stream ${error.name} ${track.kind} [error: %o]${JSON.stringify(error)}`);
              }
            }
            else
            {
          ...
          if (RTCPeerConnection.prototype.addTrack)
          {
            if (this._localMediaStream.getVideoTracks()[0])
            {
              ...
              if (!trackAlreadyAdded)
              {
                ...
                {
                  logger.warn(`${this._id} _processInDialogSdpOffer() failed no stream ${error.name} ${this._localMediaStream.getVideoTracks()[0].kind} [error: %o]${JSON.stringify(error)}`);
                }
              }
              else
              {
      ...
      .then(() =>
      {
        ...
        return this._createLocalDescription('answer', this._rtcAnswerConstraints)
          .catch((error) =>
          {
            ...
            logger.warn(`${this._id} emit "peerconnection:createtelocaldescriptionfailed" [error:%o]${JSON.stringify(error)}`);
      ...
      .catch((error) =>
      {
        ...
        logger.warn(`${this._id} _processInDialogSdpOffer() failed [error: %o]${JSON.stringify(error)}`);
      });
  ...
  _sdpAddMid(sdp)
  {
    if (sdp.indexOf('a=mid:0') === -1)
    {
      ...
      logger.debug(`${this._id} new sdp: `, newSdp);
  ...
  _generateAnEmptyAudioTrack()
  {
    ...
    audio.play().catch((error) => { logger.error(`${this._id} new Audio() error: ${JSON.stringify(error)}`); });
  ...
  _replaceAudioToMic()
  {
    ...
    navigator.mediaDevices.getUserMedia({ audio: this._inviteMediaConstraints.audio || true, video: false })
      ...
      {
        // 获取麦克风失败，重新获取
        logger.error(`${this._id} replaceAudioToMic error: ${JSON.stringify(error)}`);
  ...
  _replaceCanvasToVideo()
  {
    ...
    navigator.mediaDevices.getUserMedia({ audio: false, video: this._inviteMediaConstraints.video || true })
      ...
      {
        // 获取摄像头失败，重新获取
        logger.error(`${this._id} replaceCanvasToVideo error: ${JSON.stringify(error)}`);
