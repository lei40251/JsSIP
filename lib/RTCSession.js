...
module.exports = class RTCSession extends EventEmitter
{
  ...
  constructor(ua)
  {
    ...
    // videoShare 用于分享视频
    this._videoMediaStream = null;
    this._videoMediaStreamLocallyGenerated = false;
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    // 设置媒体所用带宽 （单位：kbit/s）
    const bandwidth = Utils.cloneObject(options.bandwidth, {
      audio : 128,
      video : 1024
    });
    ...
    if (this._ua.sk && (this._ua.sk[8].split(';').indexOf(target.host) == -1))
    {
      this._ua.emit('failed', {
        originator : 'local',
        message    : null,
        cause      : JsSIP_C.SDK_AUTHENTICATION_ERROR.AUTH_SIPPROXY_ERROR
      });
    ...
    else
    {
      this._sendInitialRequest(
        mediaConstraints, rtcOfferConstraints, mediaStream, bandwidth);
    }
  }
  ...
  {
    logger.debug('switchVideoStream()');
  ...
  */
  ...
  {
    logger.debug('videoShare() | start video share');

    if (!videoStream)
    {
      logger.warn('no videoStream');

      return;
    }

    // 分享桌面时不支持再分享视频
    if (this._displayMediaStreamLocallyGenerated)
    {
      logger.warn('displayShare is running.');

      return;
    }

    this._videoMediaStreamLocallyGenerated = true;

    this._videoMediaStream = videoStream;
  ...
  */
  displayShare(type)
  {
    logger.debug('displayShare() | start display share');
  ...
  unVideoShare()
  {
    logger.debug('unVideoShare() | closing video share');

    if (this._videoMediaStream && this._videoMediaStreamLocallyGenerated)
    {
      this._videoMediaStreamLocallyGenerated = false;

      Utils.closeMediaStream(this._videoMediaStream);
  ...
  unDisplayShare(type)
  {
    logger.debug('unDisplayShare() | closing display share');
  ...
  _close()
  {
    ...
    if (this._displayMediaStream && this._displayMediaStreamLocallyGenerated)
    {
      logger.debug('close() | closing display MediaStream');
  ...
  _createLocalDescription(type, constraints, bandwidth)
  {
    ...
    return Promise.resolve()
      ...
      .then((desc) =>
      {
        ...
        if (type === 'offer')
        {
          ...
          if (sdp.media[1])
          {
            let fH264=true;
            const payloadArr=[];
            const videoMedia = sdp.media.filter((media) => media.type === 'video');

            videoMedia[0].rtp.forEach((rtp) => 
            {
              if (rtp.codec==='ulpfec')
              {
              ...
              if (rtp.codec.toLowerCase() === 'h264'&&fH264)
              {
                fH264 = false;
                payloadArr.push(rtp.payload);
              }
            ...
            videoMedia[0].fmtp.forEach((fmtp) => 
            {
            ...
            sdp.media[1].payloads = payloadArr.join(' ');
            sdp.media[1].fmtp && (sdp.media[1].fmtp = sdp.media[1].fmtp.filter((r) => payloadArr.indexOf(r.payload) !== -1));
            sdp.media[1].rtp && (sdp.media[1].rtp = sdp.media[1].rtp.filter((r) => payloadArr.indexOf(r.payload) !== -1));
            sdp.media[1].rtcpFb && (sdp.media[1].rtcpFb = sdp.media[1].rtcpFb.filter((r) => payloadArr.indexOf(r.payload) !== -1));

            desc.sdp = sdp_transform.write(sdp);
            // desc.sdp = desc.sdp.replace(/a=rtcp-fb:127 goog-remb\r\n/, 'a=rtcp-fb:127 ccm tmmbr\r\n');
      ...
      .then(() =>
      {
          ...
          (connection.iceGatheringState === 'gathering' && this._iceReady))
        {
          ...
          this._toggleCallMode(connection.localDescription.sdp, 'local');
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    for (const m of sdp.media)
    {
      if (m.type === 'video' && m.direction !== 'sendonly' && m.direction !== 'inactive')
      {
        addVideo = true;
        break;
      }
   ...
   */
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints, mediaStream, bandwidth)
  {
    const request_sender = new RequestSender(this._ua, this._request, {
      onRequestTimeout : () =>
      {
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        return this._createLocalDescription('offer', rtcOfferConstraints, bandwidth)
          .catch((error) =>
          {
  ...
  _receiveInviteResponse(response)
  {
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._toggleCallMode(e.sdp, 'local');
  ...
  _sendReinvite(options = {})
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then((sdp) =>
      {
        ...
        this._toggleCallMode(sdp, 'local');
  ...
  // 切换通话模式
  _toggleCallMode(sdp, originator)
  {
    logger.debug('callMode()');
    ...
    for (const m of sdp.media)
    {
      if (m.type === 'video' && m.direction !== 'sendonly' && m.direction !== 'inactive' && m.port !== 0)
      {
        callmode = 'video';
        break;
      }
