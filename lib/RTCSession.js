...
module.exports = class RTCSession extends EventEmitter
{
    ...
    return Promise.resolve()
      ...
      .then(async() =>
      {
        ...
        if (mediaStream)
        {
          ...
          {
            if ((track.kind === 'audio' && Boolean(this._inviteMediaConstraints.audio)) || (track.kind === 'video' && Boolean(this._inviteMediaConstraints.video)))
            {
              throw Error(`There are two ${this._inviteMediaConstraints.audio ? 'audio' : 'video'} tracks in the input, please check the parameters`);
            }
      ...
      return navigator.mediaDevices.getDisplayMedia({ video: { width: { max: 1920 }, height: { max: 1080 }, frameRate: 15 }, audio: true })
        ...
        .catch((error) =>
        {
          if (error.message || error.message.indexOf('user gesture handler') !== -1)
          {
            ...
            {
              // BFCP 释放资源
              this._localShareStreamLocallyGenerated || this._sendFloorRelease();
            }, 10000);
          }
          else
          {
  ...
  _createLocalDescription(type, constraints)
  {
    ...
    return Promise.resolve()
      ...
      .then((desc) =>
      {
        ...
        // 非BFCP修改为只支持 42c01e
        this._enableBFCP || (desc.sdp = desc.sdp.replace(/42e01f/g, '42c01e'));
      ...
      .then((sdp) =>
      {
        ...
        if (type === 'offer')
        {
          ...
          for (const m of sdp_desc.media)
          {
            ...
            console.warn('mptm: ', this._mode);
          }

          console.warn('this.mode: ', this._mode);
        ...
        else
        {
          ...
          for (const m of sdp_desc.media)
          {
            console.warn('aaaaammmmmmmmmmm: ', m.type);
            if (m.type !== 'video')
            {
              continue;
            }
            ...
            console.warn('mmmmmm: ', this._localToAudio, this._localToVideo, this._remoteToAudio);
            ...
            else if (!this._remoteToAudio || this._localToVideo)
            {
              this._ontogglemode('video');
            }
          }
          ...
          if (media.type === 'video')
          {
            ...
            else
            {
              ...
              media.fmtp.forEach((fmtp) =>
              {
                ...
                {
                  fmtp.config = `x-google-start-bitrate=${Math.floor(bwidth * .6)};x-google-min-bitrate=${Math.floor(bwidth * .3)};x-google-max-bitrate=${Math.floor(bwidth)};${fmtp.config}`;
                }
              });
            }
          }
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    const sdp1 = sdp_transform.parse(request.body);
    ...
    const newSdp = this._sdpAddMid(sdp_transform.write(sdp1));
    const e = { originator: 'remote', type: 'offer', sdp: newSdp };
  ...
  _receiveInviteResponse(response)
  {
    ...
    switch (true)
    {
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        const newSdp = this._sdpAddMid(sdp_transform.write(sdp));
        // const newSdp = this._sdpAddMid(response.body);
        const e = { originator: 'remote', type: 'answer', sdp: newSdp };
  ...
  _sendReinvite(options = {})
  {
    ...
    function onSucceeded(response)
    {
      ...
      const newSdp = this._sdpAddMid(sdp_transform.write(sdp_body));
      const e = { originator: 'remote', type: 'answer', sdp: newSdp };
