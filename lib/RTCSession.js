...
module.exports = class RTCSession extends EventEmitter
{
      ...
      .then((stream) =>
      {
        ...
        if (stream)
        {
          stream.getTracks().forEach((track) =>
          {
            if (track.kind === 'video')
            {
              this._videoRtpSender = this._connection.addTrack(track, stream);
            }
            else
            {
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    const sdp = request.parseSDP();
    const mediaConstraints={ audio:false, video:false };
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then((stream) =>
      {
        ...
        else
        {
          ...
          this._connection.removeTrack(this._videoRtpSender);
