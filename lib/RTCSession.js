    ...
    try
    {
      ...
      if (resolvedOptions.sources instanceof Array && resolvedOptions.sources.length > 0)
      {
        for (let index = 0; index < resolvedOptions.sources.length; index++)
        {
          ...
          if (Object.prototype.hasOwnProperty.call(sourceOptions, 'aiBackground'))
          {
            if (sourceOptions.aiBackground)
            {
              composer.setAiBackground(index, sourceOptions.aiBackground);
            }
            else
            {
              composer.clearAiBackground(index);
            }
          }
        }
      }
      ...
      return Promise.resolve()
        ...
        .then(async({ constraints: videoConstraints, useComposerBranch }) => 
        {
          ...
          try
          {
            ...
            if (typeof currentComposer.getAiBackground === 'function')
            {
              try
              {
                const aiBackground = currentComposer.getAiBackground(0);

                if (aiBackground)
                {
                  sourceOptions.aiBackground = aiBackground;
                }
              }
              catch (error)
              {}
 ...
 */
  ...
  {
    this._statsMonitor && this._statsMonitor.markChange(reason);
  }
