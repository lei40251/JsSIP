...
module.exports = class RTCSession extends EventEmitter
{
      ...
      return Promise.resolve()
          ...
          this._connection.getSenders().find((s) =>
          {
            ...
            if (s.track && s.track.kind == 'video')
            {
              if (this._enableBFCP)
              {
                ...
                (s.track != this._bfcpVideoTrack && s.track != this._localShareStream.getVideoTracks()[0]) && s.track.stop();
              }
              else
              {
                s.track.stop();
        ...
        .then((stream) =>
        {
          ...
          const sender = this._connection.getSenders().find((s) =>
          {
            ...
            {
              // 启用了BFCP，区分一下BFCP控制的视频轨道
              return s.track.kind == 'video' && s.track != this._bfcpVideoTrack && s.track != this._localShareStream.getVideoTracks()[0];
            }
            else
            {
  ...
  async share(type, id, assembly, dual, skip)
  {
    ...
    else if (type === 'screen')
    {
      ...
      // 分享屏幕 默认帧率 15
      return navigator.mediaDevices.getDisplayMedia({ video: { width: { max: 1920 }, height: { max: 1080 }, frameRate: 15 } })
        .then((stream) =>
        {
          this._localShareRTPSender = null;
        ...
        if (this._enableBFCP)
        {
          ...
          this._connection.getTransceivers().forEach((transceiver) =>
          {
            ...
            const isCanvasTrack = () =>
            {
              ...
              return settings || settings.deviceId === 'canvas' ||
  ...
  _dataChannelSend(message, transactionId)
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      // 输出日志：发送消息次数及tid，时间戳
      logger.debug(`BFCP send: ${JSON.stringify(Utils.uint8ArrayToBase64(messageState.message))} ${messageState.retries + 1}, ${transactionId} ${Date.now()}`);
