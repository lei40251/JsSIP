      ...
      .then((desc) =>
      {
        if (type === 'offer')
        {
          ...
          sdp.media.forEach((media, index) =>
          {
            ...
            if (media.type === 'video')
            {
              media.bandwidth = [ { type: 'AS', limit: 1024 }, { type: 'RR', limit: 6000 }, { type: 'RS', limit: 8000 } ];
              media.ext = [ { value: 13, uri: 'urn:3gpp:video-orientation' } ];
              media.invalid = [ { value: 'tcap:1 RTP/AVPF' }, { value: 'pcfg:1 t=1' } ];
            }
            else if (media.type === 'audio')
            {
              media.bandwidth = [ { type: 'AS', limit: 100 }, { type: 'RR', limit: 600 }, { type: 'RS', limit: 2000 } ];
            }
            // }
          });
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .then(() => response.getHeader('require') === '100rel' && this._earlyDialogs[Object.keys(this._earlyDialogs)[0]].sendRequest(CRTC_C.PRACK))
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        if (!response.body)
        {
          ...
          this.sendRequest(CRTC_C.ACK, { requestNoBody: true });
