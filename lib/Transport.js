...
module.exports = class Transport
{
  ...
  _onData(data)
  {
    ...
    // 第一步，修复BFCP用到的SDP信息
    const updatedRtcpMessage = data.replace('UDP/DTLS/SCTP/BFCP *', 'UDP/DTLS/SCTP webrtc-datachannel');

    // 检查第一步是否成功，成功后修改Content-Length的值
    if (updatedRtcpMessage !== data)
    {
      data = updatedRtcpMessage.replace(
        /(Content-Length:\s*)(\d+)/,
        (_, p1, p2) =>
        {
          // 将原始值加 12
          const newValue = parseInt(p2, 10) + 12;
