...
module.exports = class Transport
{
  ...
  send(data)
  {
    ...
    message = message.replace(/^m=(\w+)\s9\s/gm, (match, media) =>
    {
      return `m=${media} ${port++} `;
    });
  ...
  _onData(data)
  {
    ...
    // 删除bundle
    data = data.replace(/a=group:BUNDLE.*\r\n/, '');
    // 统一修改收到的SDP
    data = data.replace(/b=(AS|RS|RR):\d.*\r?\n/g, '');
    // 修改端口和方向，适配音视频切换问题
    data = Utils.fixVideoInactive(data);

    // 适配部分手机视频切换为音频模式时没有pt值
    data = data.replace(/^(m=video\s+0\s+.*AVPF)$/gm, '$1 0');
    ...
    if (data.includes('UDP/DTLS/SCTP webrtc-datachannel'))
    {
      ...
      {
        data = data.replace(/(SCTP webrtc-datachannel\r\n)/g, `$1a=sctp-port:${this._sctp_port||0}\r\na=max-message-size:${this._max_message_size||1073741823}\r\n`);
      }
    }
