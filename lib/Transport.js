...
const debug = require('debug')('FlyInn:Transport');
const debugerror = require('debug')('FlyInn:ERROR:Transport');
...
module.exports = class Transport
{
  constructor(sockets, recovery_options = C.recovery_options)
  {
    ...
    sockets.forEach(function(socket)
    {
      ...
      {
        throw new TypeError('Invalid argument.' +
                            ' invalid \'FlyInn.Socket\' instance');
      }
  ...
  connect()
  {
    ...
    if (this.isConnected())
    {
      ...
      log.debug('Transport is already connected');
    ...
    else if (this.isConnecting())
    {
      ...
      log.debug('Transport is connecting');
  ...
  send(data)
  {
    ...
    if (!this.isConnected())
    {
      ...
      log.error('unable to send message, transport is not connected');
  ...
  _reconnect()
  {
    ...
    log.debug(`reconnection attempt: ${this.recover_attempts}. next connection attempt in ${k} seconds`);
  ...
  _onData(data)
  {
    ...
    if (data === '\r\n')
    {
      ...
      log.debug('received message with CRLF Keep Alive response');
    ...
    else if (typeof data !== 'string')
    {
      ...
      catch (evt)
      {
        debug('received binary message failed to be converted into string,' +
              ' message discarded');
        log.debug('received binary message failed to be converted into string,' +
              ' message discarded');
