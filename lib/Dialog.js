...
const debug = require('debug')('FlyInn:Dialog');
...
module.exports = class Dialog
{
  ...
  constructor(owner, message, type, state = C.STATUS_CONFIRMED)
  {
    ...
    log.debug(`new ${type} dialog created with status ${this._state === C.STATUS_EARLY ? 'EARLY': 'CONFIRMED'}`);
  }
  ...
  update(message, type)
  {
    ...
    log.debug(`dialog ${this._id.toString()}  changed to CONFIRMED state`);
  ...
  terminate()
  {
    ...
    log.debug(`dialog ${this._id.toString()} deleted`);
