...
module.exports = class Dialog
{
  ...
  sendRequest(method, options = {})
  {
    ...
    // 增加适配 100rel 200ok nobody情况下的cseq
    const requestNoBody = options.requestNoBody || false;
    const request = this._createRequest(method, extraHeaders, body, requestNoBody);
  ...
  // RFC 3261 12.2.1.1.
  _createRequest(method, extraHeaders, body, requestNoBody)
  {
    ...
    {
      extraHeaders.push(`RAck: 1 ${ this.local_seqnum } INVITE`);
    }

    // 200ok no body 情况下的cseq值
    const cseq = (method === CRTC_C.CANCEL || method === CRTC_C.ACK) ? requestNoBody ? this._local_seqnum - 1 :
      this._local_seqnum : this._local_seqnum += 1;
  ...
  _checkInDialogRequest(request)
  {
    ...
    if (request.method === CRTC_C.INVITE ||
        (request.method === CRTC_C.UPDATE && request.body))
    {
