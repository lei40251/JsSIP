...
class NonInviteClientTransaction extends EventEmitter
{
  constructor(ua, transport, request, eventHandlers)
  {
    ...
    via += ` ${ua.configuration.via_host};rport;branch=${this.id}`;
...
class InviteClientTransaction extends EventEmitter
{
  ...
  sendACK(response)
  {
    const ack = new SIPMessage.OutgoingRequest(CRTC_C.ACK, this.request.ruri,
      this.ua, {
  ...
  cancel(reason)
  {
    ...
    const cancel = new SIPMessage.OutgoingRequest(CRTC_C.CANCEL, this.request.ruri,
      this.ua, {
        'route_set' : this.request.getHeaders('route'),
        'call_id'   : this.request.getHeader('call-id'),
        'cseq'      : this.request.cseq
