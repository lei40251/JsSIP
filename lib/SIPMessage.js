...
class OutgoingRequest
{
  constructor(method, ruri, ua, params, extraHeaders, body)
  {
    ...
    const call_id = params.call_id ||
      (ua.configuration.crtc_id + Utils.createRandomToken(15));
  ...
  toString()
  {
    ...
    const userAgent = this.ua.configuration.user_agent || CRTC_C.USER_AGENT;

    // 增加100rel支持, OPTIONS 消息不添加 Supported 头
    // Allow.
    msg += `Allow: ${CRTC_C.ALLOWED_METHODS}\r\n`;
    this.method !== CRTC_C.OPTIONS && (msg += `Supported: ${supported},100rel\r\n`);
...
class InitialOutgoingInviteRequest extends OutgoingRequest
{
  ...
  {
    super(CRTC_C.INVITE, ruri, ua, params, extraHeaders, body);
...
class IncomingRequest extends IncomingMessage
{
  ...
  */
  reply(code, reason, extraHeaders, body, onSuccess, onFailure)
  {
    ...
    const allow_events = [ 'talk', 'hold', 'conference', 'refer', 'check-sync' ];
    ...
    {
      response += `Allow: ${CRTC_C.ALLOWED_METHODS}\r\n`;
      response += `Accept: ${CRTC_C.ACCEPTED_BODY_TYPES}\r\n`;
    }
    else if (code === 405)
    {
      response += `Allow: ${CRTC_C.ALLOWED_METHODS}\r\n`;
    ...
    // for 3PCC notify event
    response += `Allow-Events: ${allow_events}\r\n`;
