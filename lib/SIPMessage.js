...
const debug = require('debug')('FlyInn:SIPMessage');
...
class OutgoingRequest
{
  constructor(method, ruri, ua, params, extraHeaders, body)
  {
    ...
    const call_id = params.call_id ||
      (ua.configuration.flyinn_id + Utils.createRandomToken(15));
  ...
  */
  parseHeader(name, idx = 0)
  {
    ...
    if (!this.headers[name])
    {
      ...
      log.debug(`header "${name}" not present`);
    ...
    else if (idx >= this.headers[name].length)
    {
      ...
      log.debug(`not so many "${name}" headers present`);
    ...
    if (parsed === -1)
    {
      ...
      log.debug(`error parsing "${name}" header field with value "${value}"`);
