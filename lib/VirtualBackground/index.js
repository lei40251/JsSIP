...
const DEFAULT_CONFIG = {
  video        : { width: 1280, height: 720, targetFps: 15, mirror: false },
  segmentation : {
    backend         : 'wasmSimd',
...
module.exports = class VirtualBackgroundEngine
{
  ...
  mergeConfig(options)
  {
    ...
    {
      const postProcessing = Object.assign({}, options.postProcessing);
      const jointBilateralFilter = postProcessing.jointBilateralFilter;
      ...
      backgroundEl.onload = () =>
      {
        try
        {
          ...
          this.pipeline = buildWebGL2Pipeline(
            sourcePlayback,
            this.backgroundEl,
            { type: type, mirror: this.config.video.mirror },
  ...
  clearBackground()
  {
    ...
    const gl = this.processingCanvas.getContext('webgl2');
    ...
    const { videoEl, processingCanvas } = this;
