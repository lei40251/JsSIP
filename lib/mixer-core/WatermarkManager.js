...
class WatermarkManager
{
  ...
  _createTextSurface(watermark)
  {
    ...
    const measured = context.measureText ? context.measureText(text).width : (text.length * watermark.fontSize * 0.6);
    const width = Math.max(1, Math.ceil(measured + (watermark.padding * 2)));
    const height = Math.max(1, Math.ceil((watermark.fontSize * 1.35) + (watermark.padding * 2)));
