  ...
  return Promise.resolve()
    ...
    .then((cameras) =>
    {
      cameras.forEach((cam, index) =>
      {
        ...
        cams.push({ kind: cam.kind, label: cam.label == ''?label:cam.label, deviceId: cam.deviceId });
      });
...
exports.getMicrophones = () =>
{
  ...
  return Promise.resolve()
    ...
    .then((microphones) =>
    {
      ...
      {
        const label = `microphone ${ index + 1}`;

        mics.push({ kind: mic.kind, label: mic.label == ''?label:mic.label, deviceId: mic.deviceId });
      });
...
exports.getSpeaker = () =>
{
  ...
  return Promise.resolve()
    ...
    .then((devices) =>
    {
      devices.forEach((speaker, index) =>
      {
        const label = `speaker ${ index + 1}`;

        speakers.push({ kind: speaker.kind, label: speaker.label == ''?label:speaker.label, deviceId: speaker.deviceId });
      });
 ...
 */
exports.getStreams = (pc, type, streamIndex) =>
{
  ...
  if (type === 'remote' && RTCPeerConnection.prototype.getReceivers)
  {
    ...
    result = { audioStream: audioStream, videoStream: videoStream, mediaStream: mediaStream };
  }
  else if (type === 'slides' && RTCPeerConnection.prototype.getReceivers)
  {
    if (streamIndex)
    {
      ...
      result = { videoStream: videoStream, mediaStream: mediaStream };
    }
  }
  else if (type === 'local' && RTCPeerConnection.prototype.getSenders)
  {
    ...
    result = { audioStream: audioStream, videoStream: videoStream };
  }
  else
  {
...
const createCanvasVideoTrack = function()
{
  const canvas = document.createElement('canvas');
  ...
  canvas.width = 60;
  canvas.height = 40;
  const videoStream = canvas.captureStream(1); // 帧率1
