...
exports.generateAnBlackVideoTrack = (options) =>
{
  ...
  const svgSource = options.svgSource || null;
  ...
  const drawToCanvas = function()
  {
    ...
    if (hidden)
    {
      timeoutId = setTimeout(drawToCanvas, 1000);
    }
    else
    {
      animationId = window.requestAnimationFrame(drawToCanvas);
    }
  };
...
exports.compatiblePayload = (sdp, newPayload = 124) =>
{
  ...
  // 取得浏览器生成的payload
  const payloadMatch = sdp.match(/a=fmtp:(\d+) .*packetization-mode=0/);
  const currPayload = payloadMatch ? payloadMatch[1] : '';
...
exports.getApplicationMediaPositions = (sdp) =>
{
  ...
  mediaSections.forEach((section, index) =>
  {
    ...
    // 为音频和视频媒体添加rtcp-mux
    if (/^m=(audio|video)/.test(section) && !section.includes('a=rtcp-mux'))
    {
      updatedSection = updatedSection.replace(/(\r\n|\r|\n)/, '$1a=rtcp-mux\r\n');
    }
 ...
 * 判断当前环境是否为微信内置浏览器。
 *
 * @returns {boolean} 微信浏览器时返回 true。
 */
exports.isWeChat = () =>
{
  return typeof navigator !== 'undefined' && navigator.userAgent.indexOf('WeChat') !== -1;
};

/**
 * 判断当前是否为 iOS 15.1 或 15.2。
 *
 * 这两个版本存在 WebKit canvas crash bug：
 * https://bugs.webkit.org/show_bug.cgi?id=232006
 *
 * @returns {boolean} 命中 iOS 15.1/15.2 时返回 true。
 */
exports.isIOS151_152 = () =>
{
  ...
  const ua = navigator.userAgent.toLowerCase().match(/cpu iphone os (.*?) like mac os/);

  return ua && ua[1] && (ua[1].includes('15_1') || ua[1].includes('15_2'));
};

/**
 * 绕过 iOS 15.1/15.2 的 WebKit canvas crash bug。
 *
 * 如果是受影响的 iOS 版本，则通过 canvas captureStream 方式重新获取流，
 * 避免直接使用原始视频轨道导致的崩溃。
 *
 * @param {MediaStream} stream - 原始媒体流。
 * @returns {MediaStream} 绕过处理后的媒体流（非受影响版本则原样返回）。
 */
exports.bypassIOS151_152CanvasBug = (stream) =>
{
  return exports.isIOS151_152() ? exports.getStreamThroughCanvas(stream) : stream;
};

/**
 * 判断 UA 是否启用了 5G VoLTE 服务。
 *
 * 授权码中 sk[7] >= 3 表示 5G 模式，需要在 SDP 中携带
 * MMTEL 服务头并进行相应的 SDP 差异化处理。
 *
 * @param {Object} ua - UA 实例。
 * @returns {boolean} 5G 模式时返回 true。
 */
exports.is5GService = (ua) =>
{
  return ua && ua.sk && ua.sk[7] >= 3;
};
 ...
 *
 * @param {Object} ua - UA 实例。
 * @returns {boolean} 支持视频时返回 true。
 */
exports.supportsVideo = (ua) =>
{
  return ua && ua.sk && Number(ua.sk[7]) >= 1;
};

/**
 * 判断 UA 授权是否支持音频升级到视频。
 *
 * 授权码中 sk[7] >= 2 表示允许 upgradeToVideo。
 *
 * @param {Object} ua - UA 实例。
 * @returns {boolean} 支持升级时返回 true。
 */
exports.supportsUpgradeToVideo = (ua) =>
{
  return ua && ua.sk && Number(ua.sk[7]) >= 2;
};
...
exports.shouldRecoverVideoFrameRateByUA = () =>
{
  ...
  const isAndroidWeChat = /android/i.test(ua) && /micromessenger/i.test(ua);
  const isHarmonyWeChat = /micromessenger/i.test(ua) && /(OpenHarmony|HarmonyOS)/i.test(ua);
  const isHarmonyNativeBrowser = /huaweibrowser/i.test(ua) && /(OpenHarmony|HarmonyOS)/i.test(ua);

  return isAndroidWeChat || isHarmonyWeChat || isHarmonyNativeBrowser;
};
...
exports.replaceDtmfPayloads = (sdp, payloadMappings) =>
{
  ...
  for (const line of lines)
  {
    ...
    else
    {
      ...
      // 尝试匹配 m=audio 行
      const audioMLineMatch = line.match(/^(m=audio\s+\d+\s+.*AVPF\s+)(.*)/);
...
exports.sendKeyFrames = (pc, interval, frequency) =>
{
   ...
   * 请求单个 sender 产生关键帧
   */
  const requestKeyFrame = async(sender) =>
  {
    if (!sender)
    {
      return;
    ...
    if (typeof sender.generateKeyFrame === 'function')
    {
      ...
      {
        console.warn(
          '[sendKeyFrames] generateKeyFrame failed:',
          error.toString()
        );
      }
    }
    ...
    try
    {
      ...
      setTimeout(async() =>
      {
        try
        {
          ...
          {
            item.encoding.scaleResolutionDownBy =
              item.oldScale;
          }
    ...
    {
      console.warn(
        '[sendKeyFrames] scaleResolutionDownBy fallback failed:',
        error.toString()
      );
    }
  };
...
exports.getEnvironmentId = (devices) =>
{
   ...
   */
  const getEnvironmentIdFromDevices = (deviceList) =>
  {
    const environments = [];
  ...
  return navigator.mediaDevices.enumerateDevices()
    .then(getEnvironmentIdFromDevices);
};
...
exports.isVideoTrackHealthy = (mediastream) =>
{
  ...
  const result = (video.videoWidth && video.videoHeight) ? true : false;
...
exports.updateSdpByConstraints=(sdp, constraints) =>
{
  ...
  {
    // 判断当前段是音频还是视频
    const isAudio = section.startsWith('m=audio');
    const isVideo = section.startsWith('m=video');
...
exports.fixVideoInactive = (sdp) =>
{
  return sdp.replace(
    ...
    {
      // 获取换行符格式
      const EOL = block.match(/\r?\n/) ? block.match(/\r?\n/)[0] : '\r\n';

      const hasInactive = /a=inactive/.test(block);
      const mLineMatch = block.match(/m=video (\d+)/);

      if (!mLineMatch) return block;

      let port = parseInt(mLineMatch[1], 10);
      let newBlock = block;

      // ① a=inactive 且端口非0 => 端口置0
      if (hasInactive && port !== 0)
      {
        newBlock = newBlock.replace(/m=video \d+/, 'm=video 0');
        port = 0; // 更新局部变量，以便后续逻辑连贯
      }

      // ② 端口为0 (包含刚才被置为0的情况) 且 没有inactive => 补 inactive
      // 注意：这里我们使用 replace 替换现有的方向属性，防止冲突
      if (port === 0)
      {
        if (hasInactive)
        {
          // 已经是 inactive 了，不做操作 (或者你可以选择清理 sendrecv)
        }
        // 检查是否有其他方向属性，如果有则替换，没有则追加
        else if (/a=(sendrecv|sendonly|recvonly)/.test(newBlock))
        {
          newBlock = newBlock.replace(/a=(sendrecv|sendonly|recvonly)/, 'a=inactive');
        }
        else
        {
          // 插在 m 行的下一行
          newBlock = newBlock.replace(/(m=video[^\r\n]*)/, `$1${EOL}a=inactive`);
        }
      }
...
{
  // 1. 修改 m=video 的端口为 0
  let newSdp = sdp.replace(/(m=video\s+)\d+/, '$10');

  // 2. 修改 video 部分的方向为 inactive
  // 使用类似的逻辑，防止跨越到下一个 m= 行
