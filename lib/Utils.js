...
// filter SDP
exports.filterSdpMedia=(sdpObj, filterCondition) =>
{
  const mediaArr = sdpObj.media;
  ...
  mediaArr.forEach(function(media) {
    ...
    // filter candidate
    if (candidateFilter) {
      media.candidates = media.candidates.filter(
