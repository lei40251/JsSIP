...
const FloorRequestId = require('../AttributeClasses.js').FloorRequestId;
const FloorRequestStatusAtr = require('../AttributeClasses.js').FloorRequestStatus;
...
const RequestStatus = require('../AttributeClasses.js').RequestStatus;
...
class Parser
{
  ...
  {
    const primitive = parseInt(commonHeader.substring(8, 16), 2);
    const payloadLength = parseInt(commonHeader.substring(16, 32), 2);
    const conferenceId = parseInt(commonHeader.substring(32, 64), 2);
    const transactionId = parseInt(commonHeader.substring(64, 80), 2);
    const userId = parseInt(commonHeader.substring(80, 96), 2);
  ...
  static _parseAttributes(attributes)
  {
    ...
    while (attributes != '')
    {
      const type = parseInt(attributes.substring(0, 7), 2);
      const length = parseInt(attributes.substring(8, 16), 2);
      const attribute = attributes.substring(0, length * 8);
      // let content;
  ...
  static _parseSupportedAttributes(content)
  {
    ...
    {
      const binType = content.substring(8 * (i - 1), 8 * i);
  ...
  static _parseSupportedPrimitives(content)
  {
    ...
    for (let i = 1; i < (content.length / 8) + 1; i++)
    {
      const binPrimitive = content.substring(8 * (i - 1), 8 * i);
  ...
  static parseMessage(message)
  {
    try
    {
      ...
      const commonHeader = Parser._parseCommonHeader(binaryMessage.substring(0, 96));
      const attributes = Parser._parseAttributes(binaryMessage.substring(96));
      ...
      switch (commonHeader.primitive)
      {
        ...
        {
          const floorRequest = new FloorRequest();
        ...
        {
          const floorRequestStatus = new FloorRequestStatusMsg();
        ...
        {
          const floorRelease = new FloorRelease();
        ...
        {
          const floorRequestStatusAck = new FloorRequestStatusAck();
        ...
        {
          const floorStatusAck = new FloorStatusAck();
        ...
        {
          const floorQuery = new FloorQuery();
