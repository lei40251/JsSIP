...
const AttrName = require('../AttributeName.js');
...
class User
{
  ...
  // FloorRequest 请求
  floorRequestMessage(transactionId, floorId)
  {
    const floorRequest = new FloorRequest(this.conferenceId, transactionId, this.userId, floorId);

    return Buffer.from(floorRequest.encode());
  }

  // FloorRelease 请求
  floorReleaseMessage(transactionId, floorRequestId)
  {
    const floorRelease = new FloorRelease(this.conferenceId, transactionId, this.userId, floorRequestId);

    return Buffer.from(floorRelease.encode());
  }

  // FloorRequestStatusAck 请求
  floorRequestStatusAckMessage(floorRequestStatusMessage)
  {
    const wantedFloorId = floorRequestStatusMessage.getAttribute('FloorRequestInformation').content[1].content[0];
    const floorRequestStatusAck = new FloorRequestStatusAck(this.conferenceId,
      floorRequestStatusMessage.commonHeader.transactionId, this.userId, wantedFloorId);
   ...
   * @public
   */
  floorStatusMessage(floorId, requestStatus, transactionId)
  {
    ...
    let ctid = this.currentTransactionId++;

    if (transactionId)
    {
      ctid = transactionId;
    }
    const floorStatus = new FloorStatus(this.conferenceId, ctid, this.userId, User.getFloorRequestId(), floorId, requestStatus);
   ...
   * @public
   */
  floorStatusAckMessage(floorId, floorStatusMessage)
  {
    const floorStatusAck = new FloorStatusAck(this.conferenceId, floorStatusMessage.commonHeader.transactionId, this.userId, floorId);
