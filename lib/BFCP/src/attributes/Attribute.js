...
class Attribute
{
  ...
  encode()
  {
    const type = Complements.complementBinary(this.type.toString(2), 7);
    const m = '1';
    const length = Complements.complementBinary(this.length.toString(2), 8);
    let content = null;
  ...
  _encodeGroupedAttributeContent()
  {
    ...
    for (const attribute of this.content)
    {
  ...
  _encodeOctetStringContent()
  {
    ...
    switch (this.type)
    {
      case Type.SupportedAttributes:
        for (const attributeType of this.content)
        {
      ...
      case Type.SupportedPrimitives:
        for (const primitiveType of this.content)
        {
  ...
  _encodeOctetString16Content()
  {
    switch (this.type)
    {
      case Type.RequestStatus: {
        const requestStatus = Complements.complementBinary(this.content[0].toString(2), 8);
        const queuePosition = Complements.complementBinary(this.content[1].toString(2), 8);
