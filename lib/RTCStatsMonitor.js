...
module.exports = class RTCStatsMonitor extends EventEmitter
{
   ...
   * @param {number} [options.reportIntervalMs=2000] 两个兼容事件的输出间隔
   * @param {number} [options.bgIntervalMs=2000] 页面后台时的采样间隔
   * @param {number} [options.transitionSamples=2] 媒体变化后跳过异常诊断的样本数
   * @param {boolean} [options.detailedReport=true] 是否记录常用诊断摘要并发送 detailed-report 事件
   * @param {boolean} [options.rawStatsLog=true] 是否按限频规则记录脱敏后的原始报告
   * @param {number} [options.timeoutMs=5000] 单次 getStats 超时时间，最小 100ms
  ...
  constructor(pc, options = {})
  {
    ...
    this._options.reportIntervalMs = Math.max(this._options.sampleIntervalMs, number(this._options.reportIntervalMs) || DEFAULT_OPTIONS.reportIntervalMs);
    this._options.bgIntervalMs = Math.max(this._options.sampleIntervalMs, number(this._options.bgIntervalMs) || DEFAULT_OPTIONS.bgIntervalMs);
    this._options.timeoutMs = Math.max(100, number(this._options.timeoutMs) || DEFAULT_OPTIONS.timeoutMs);
    this._options.rawLogIntervalMs = Math.max(1000, number(this._options.rawLogIntervalMs) || DEFAULT_OPTIONS.rawLogIntervalMs);
    const transitionSamples = number(this._options.transitionSamples);

    this._options.transitionSamples = transitionSamples === null
      ? DEFAULT_OPTIONS.transitionSamples
      : Math.max(0, Math.floor(transitionSamples));
   ...
   */
  markChange(reason = 'external-media-change')
  {
    this._transitionReason = reason;
    this._transitionSamples = this._options.transitionSamples;
  }

  getReport()
  {
    return cloneSnapshot(this._latestReport);
  }

  getLegacyReport()
  {
    return cloneSnapshot(this._legacyReport);
  }

  getNetworkQuality()
  {
    return cloneSnapshot(this._networkQuality);
  }
  ...
  async _sample()
  {
    ...
    try
    {
      ...
      this._sampleCount++;
      this._consecutiveErrors = 0;
      this._latestReport = detailedReport;

      if (this._options.detailedReport)
      {
        try { logger.debug('detailed-report: ', JSON.stringify(this._createLogReport(detailedReport))); }
        catch (error) { logger.warn(`detailed-report logging failed: ${error.message || String(error)}`); }
        this._safeEmit('detailed-report', this._createEventReport(detailedReport));
      }
      ...
      else if (this._lastLegacyTimestamp === null || detailedReport.timestamp - this._lastLegacyTimestamp >= this._options.reportIntervalMs)
      {
        const legacyReport = this._createLegacyReport(detailedReport);
        const networkQualityReport = this._createQualityReport(detailedReport);

        this._lastLegacyTimestamp = detailedReport.timestamp;
        this._legacyReport = legacyReport;
        this._networkQuality = networkQualityReport;
  ...
  async _collect()
  {
    ...
    const rawStats = await withTimeout(
      ...
      `getStats timed out after ${this._options.timeoutMs}ms`
    );
    ...
    const report = this._createReport(normalized.reports, getStatsDurationMs, normalized.format);

    this._updateCompat(normalized.reports, normalized.format);
    report.compatibility = this._getCompatibility();
  ...
  async _requestStats()
  {
    ...
    {
      return this._requestByCallback(error);
    }
    ...
    return this._requestByCallback();
  }
   ...
   * @private
   */
  async _requestByCallback(initialError)
  {
    this._compatibility.api.callbackGetStats = true;
    const selectorFirst = this._pc.getStats.length >= 3;
    const attemptTimeoutMs = Math.max(50, Math.min(500, Math.floor(this._options.timeoutMs / 2)));
   ...
   * @private
   */
  _createReport(reports, getStatsDurationMs, format)
  {
    const byId = new Map();
    ...
    {
      this.markChange('stats-topology-changed');
    }
    ...
    detailedReport.quality = this._createQuality(detailedReport, context);
   ...
   * 完整报告仍可通过 getReport() 获取。
   *
   * @private
   */
  _createLogReport(report)
  {
   ...
   * 完整报告可通过 getReport() 获取。
   *
   * @private
   */
  _createEventReport(report)
  {
    return {
      connection : {
        connectionState             : report.connection.connectionState,
  ...
  {
    if (!this._options.rawStatsLog)
    {
      return;
    }

    if (this._lastRawLogTimestamp !== null && timestamp - this._lastRawLogTimestamp < this._options.rawLogIntervalMs)
    {
      return;
    }
