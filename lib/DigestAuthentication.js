...
const debug = require('debug')('FlyInn:DigestAuthentication');
const debugerror = require('debug')('FlyInn:ERROR:DigestAuthentication');
  ...
  get(parameter)
  {
    switch (parameter)
    {
      ...
      default:
        ...
        log.error(`get() | cannot get "${parameter}" parameter`);
  ...
  authenticate({ method, ruri, body }, challenge, cnonce = null /* test interface */)
  {
    ...
    if (this._algorithm)
    {
      if (this._algorithm !== 'MD5')
      {
        ...
        log.error('authenticate() | challenge with Digest algorithm different than "MD5", authentication aborted');
    ...
    if (!this._nonce)
    {
      ...
      log.error('authenticate() | challenge without Digest nonce, authentication aborted');
    ...
    if (!this._realm)
    {
      ...
      log.error('authenticate() | challenge without Digest realm, authentication aborted');
    ...
    if (!this._credentials.password)
    {
      ...
      if (!this._credentials.ha1)
      {
        ...
        log.error('authenticate() | no plain SIP password nor ha1 provided, authentication aborted');
      ...
      if (this._credentials.realm !== this._realm)
      {
        ...
        log.error(`authenticate() | no plain SIP password, and stored ${this._credentials.realm} does not match the given ${this._realm}, cannot authenticate [stored:"%s", given:"%s"]`);
    ...
    if (challenge.qop)
    {
      ...
      else
      {
        ...
        log.error('authenticate() | challenge without Digest qop different than "auth" or "auth-int", authentication aborted');
    ...
    if (this._qop === 'auth')
    {
      ...
      log.debug(`authenticate() | using qop=auth [a2:"${a2}"]`);
    ...
    else if (this._qop === 'auth-int')
    {
      ...
      log.debug(`authenticate() | using qop=auth-int [a2:"${a2}"]`);
    ...
    else if (this._qop === null)
    {
      ...
      log.debug(`authenticate() | using qop=null [a2:"${a2}"]`);
    ...
    log.debug('authenticate() | response generated');
