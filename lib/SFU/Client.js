...
class Client extends EventEmitter
{
  constructor(clientConfig)
  {
    ...
    this._sockets = clientConfig.sockets || false;
    this._uri = clientConfig.uri || false;
    this._dn = null;
   ...
   * @memberof Client
   */
  join(roomId, displayName, options = { pcConfig: { iceTransportPolicy: 'all' } })
  {
    if (this._session)
    {
      return;
    ...
    post(this._callRouterUrl+callRouterPath, { roomId: roomId }, { 'authorization': this._userSig, 'X-SID': this._sdkAppId, 'X-UID': this._userId, 'X-FLG': 2 }, (res) =>
    {
      ...
      if (options.pcConfig)
      {
        this._options.pcConfig = options.pcConfig;
      }
      else if (pcConfig.iceServers[0].urls.length>0)
      {
      ...
      this._sockets && (sockets = new WebSocketInterface(this._sockets));
      ...
      const configuration={
        ...
        sockets        : sockets,
        uri            : `sip:${this._userId}@${this._uri || res.data.sfu.domain}`,
