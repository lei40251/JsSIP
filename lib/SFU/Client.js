...
function call(confId, options)
{
  ...
  function startStats(that)
  {
    ...
    statsTimer = setTimeout(() =>
    {
      ...
      self._session.connection.getStats().then((s) =>
      {
        s.forEach((report) =>
        {
          switch (report.type)
          {
            case 'remote-inbound-rtp':
              ...
              {
                stats.video.upRTT = report['roundTripTime'];

                curStats.video.packetsSentLost = report['packetsLost'] - stats.video.packetsSentLost;
                stats.video.packetsSentLost = report['packetsLost'];
              }
              else if (report.kind === 'audio')
              {
                stats.audio.upRTT = report['roundTripTime'];

                curStats.audio.packetsSentLost = report['packetsLost'] - stats.audio.packetsSentLost;
                stats.audio.packetsSentLost = report['packetsLost'];
              }
            ...
            case 'remote-outbound-rtp':
              if (report.kind === 'video')
              {
                stats.video.downRTT = report['roundTripTime'];
              }
              else if (report.kind === 'audio')
              {
                stats.audio.downRTT = report['roundTripTime'];
              }
            ...
            case 'outbound-rtp':
              if (report.kind === 'video')
              {
                curStats.video.packetsSent = report['packetsSent'] - stats.video.packetsSent;
                curStats.video.bytesSent = report['bytesSent'] - stats.video.bytesSent;

                stats.video.packetsSent = report['packetsSent'];
                stats.video.bytesSent = report['bytesSent'];
              }
              else if (report.kind === 'audio')
              {
                curStats.audio.packetsSent = report['packetsSent'] - stats.audio.packetsSent;
                curStats.audio.bytesSent = report['bytesSent'] - stats.audio.bytesSent;

                stats.audio.packetsSent = report['packetsSent'];
                stats.audio.bytesSent = report['bytesSent'];
              }
            ...
            case 'inbound-rtp':
              if (report.kind === 'video')
              {
                curStats.video.packetsReceived = report['packetsReceived'] - stats.video.packetsReceived;
                curStats.video.bytesReceived = report['bytesReceived'] - stats.video.bytesReceived;
                curStats.video.packetsReceivedLost = report['packetsLost'] - stats.video.packetsReceivedLost;

                stats.video.packetsReceived = report['packetsReceived'];
                stats.video.bytesReceived = report['bytesReceived'];
                stats.video.packetsReceivedLost = report['packetsLost'];
              }
              else if (report.kind === 'audio')
              {
                curStats.audio.packetsReceived = report['packetsReceived'] - stats.audio.packetsReceived;
                curStats.audio.bytesReceived = report['bytesReceived'] - stats.audio.bytesReceived;
                curStats.audio.packetsReceivedLost = report['packetsLost'] - stats.audio.packetsReceivedLost;

                stats.audio.packetsReceived = report['packetsReceived'];
                stats.audio.bytesReceived = report['bytesReceived'];
                stats.audio.packetsReceivedLost = report['packetsLost'];
              }
        ...
        eventStats.audio.upRTT = stats.audio.upRTT;
        eventStats.audio.downRTT = stats.audio.downRTT;

        eventStats.video.upRTT = stats.video.upRTT;
        eventStats.video.downRTT = stats.video.downRTT;

        // 语音上行丢包率
        if (curStats.audio.packetsSent === null)
        {
          eventStats.audio.uplinkLoss = null;
        }
        else if (curStats.audio.packetsSentLost === null)
        {
          eventStats.audio.uplinkLoss = null;
        }
        else
        {
          eventStats.audio.uplinkLoss =
            Math.floor((curStats.audio.packetsSentLost/
            (curStats.audio.packetsSentLost + curStats.audio.packetsSent))*100)/100;
        }

        // 语音上行速率
        if (curStats.audio.bytesSent === null)
        {
          eventStats.audio.uplinkSpeed = null;
        }
        else
        {
          eventStats.audio.uplinkSpeed =
          parseInt(curStats.audio.bytesSent / delay * 8000);
        }

        // 语音下行丢包率
        if (curStats.audio.packetsReceived === null)
        {
          eventStats.audio.downlinkLoss = null;
        }
        else
        if (curStats.audio.packetsReceivedLost === null)
        {
          eventStats.audio.downlinkLoss = null;
        }
        else
        {
          eventStats.audio.downlinkLoss =
            Math.floor((curStats.audio.packetsReceivedLost/
            (curStats.audio.packetsReceivedLost + curStats.audio.packetsReceived))*100)/100;
        }

        // 语音下行速率
        if (curStats.audio.bytesReceived === null)
        {
          eventStats.audio.downlinkSpeed = null;
        }
        else
        {
          eventStats.audio.downlinkSpeed =
          parseInt(curStats.audio.bytesReceived / delay * 8000);
        }

        // 视频上行丢包率
        if (curStats.video.packetsSent === null)
        {
          eventStats.video.uplinkLoss = null;
        }
        else
        if (curStats.video.packetsSentLost === null)
        {
          eventStats.video.uplinkLoss = null;
        }
        else
        {
          eventStats.video.uplinkLoss =
            Math.floor((curStats.video.packetsSentLost/
            (curStats.video.packetsSentLost + curStats.video.packetsSent))*100)/100;
        }

        // 视频上行速率
        if (curStats.video.bytesSent === null)
        {
          eventStats.video.uplinkSpeed = null;
        }
        else
        {
          eventStats.video.uplinkSpeed =
          parseInt(curStats.video.bytesSent / delay * 8000);
        }

        // 视频下行丢包率
        if (curStats.video.packetsReceived === null)
        {
          eventStats.video.downlinkLoss = null;
        }
        else
        if (curStats.video.packetsReceivedLost === null)
        {
          eventStats.video.downlinkLoss = null;
        }
        else
        {
          eventStats.video.downlinkLoss =
            Math.floor((curStats.video.packetsReceivedLost/
            (curStats.video.packetsReceivedLost + curStats.video.packetsReceived))*100)/100;
        }

        // 视频下行速率
        if (curStats.video.bytesReceived === null)
        {
          eventStats.video.downlinkSpeed = null;
        }
        else
        {
          eventStats.video.downlinkSpeed =
            parseInt(curStats.video.bytesReceived / delay * 8000);
        }

        self.emit('network-quality', eventStats);
        log.debug(JSON.stringify(eventStats));
      });
