...
function createPC(sdp, that)
{
  ...
  {
  // eslint-disable-next-line no-console
    console.log('state: ', pcRecvVideo.iceGatheringState);
    ...
    return pcRecvVideo.createAnswer({
      ...
      // Resolve right away if 'pc.iceGatheringState' is 'complete'.
        if (pcRecvVideo.iceGatheringState === 'complete')
        {
          ...
          return pcrvSdp;
        // return Promise.resolve(pcRecvVideo.localDescription.sdp);
        }
  ...
  self._session.on('sdp', (d) =>
  {
    ...
    if (d.originator === 'local')
    {
      if (this._splitSdp)
      {
        ...
        //   .then(() =>
        //   {
        //     console.log('PRESdp: ', prevSdp);
        //     const tmpSdp2= sdpTransform.parse(d.sdp);

        //     tmpSdp2.media.push(prevSdp);
        //     d.sdp=sdpTransform.write(tmpSdp2);
        //     d.sdp =
        // d.sdp.replace(/(?=a=ice-ufra)/g, `a=x-sfu-cname:${Base64.encode(JSON.stringify(cname))}\r\n`);
        //   });
    ...
    {
      // eslint-disable-next-line no-console
      console.log('remote sdp: ', sdpTransform.parse(d.sdp));
      const tmpSdp = sdpTransform.parse(d.sdp);
      ...
      if (tmpSdp.media.length>2)
      {
        ...
        d.sdp = sdpTransform.write(tmpSdp);
        // eslint-disable-next-line no-console
        console.log('d.sdp: ', d.sdp);

        tmpSdp.media = [ recvSdp ];

        // eslint-disable-next-line no-console
        console.log('recvSdp: ', sdpTransform.write(tmpSdp));

        createPC(sdpTransform.write(tmpSdp), self);
      }

      // const answer = new RTCSessionDescription({ type: 'answer', sdp: sdpTransform.write(tmpSdp) });

      // eslint-disable-next-line no-console
      // console.log('pcRecvVideo: ', pcRecvVideo);

      // eslint-disable-next-line no-console
      // pcRecvVideo.setRemoteDescription(answer)
      //   .then(() => { createPC(); })
      //   // eslint-disable-next-line no-console
