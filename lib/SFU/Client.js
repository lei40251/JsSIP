    ...
    post(this._callRouterUrl+callRouterPath, { roomId: roomId }, { 'authorization': this._userSig, 'X-SID': this._sdkAppId, 'X-UID': this._userId, 'X-FLG': 2 }, (res) =>
    {
      ...
      const pcConfig={
        ...
        iceServers         : [
          {
            urls       : [ ],
            username   : res.data.relay.auth.username,
      ...
      const configuration={
        ...
        sockets        : new WebSocketInterface(res.data.sfu.server[0]),
...
function call(roomId, options)
{
  ...
  {
    this.emit('error', data.message.reason_phrase?data.message.reason_phrase:data.cause?data.cause:'failed');
    this._session = null;
  });
