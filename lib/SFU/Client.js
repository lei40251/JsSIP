...
function createPC(pc, sdp, that)
{
  if (!pc.peer)
  {
    pc.peer = new RTCPeerConnection(/* { sdpSemantics: 'plan-b' } */);

    pc.peer.ontrack=({ track/* , streams */ }) =>
    {
      const remoteStream = new RemoteStream(track);

      // console.log('trackA: ', track);
      // track.onunmute=() =>
      // {
      if (!tracks.has(track.id))
      {
        tracks.add(track.id);
        console.log('trackB: ', track);
...
function call(roomId, options)
{
  ...
  this._session.on('sdp', (d) =>
  {
    ...
    if (d.originator === 'local')
    {
      if (this._splitSdp)
      {
        d.sdp = d.sdp.replace(/(?=a=ice-ufra)/, `a=x-sfu-cname:${Base64.encode(JSON.stringify(cname))}\r\n`);
      ...
      if (tmpSdp.media.length>2)
      {
        ...
        else
        {
          ...
          for (let i=0; i<recvMediaArr.length; ++i)
          {
            ...
            if (i>=pcArr.length)
            {
              nPCArr.push({ peer: null, media: recvMediaArr[i], ssrc_cname: ssrc_cname });
              continue;
            }
