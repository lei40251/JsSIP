...
const debugerror = require('debug')('FlyInn:ERROR:Parser');
...
exports.parseMessage = (data, ua) =>
{
  ...
  if (headerEnd === -1)
  {
    ...
    log.error('parseMessage() | no CRLF found, not a SIP message');
  ...
  if (parsed === -1)
  {
    ...
    log.error(`parseMessage() | error parsing first line of SIP message: "${firstLine}"`);
  ...
  */
  while (true)
  {
    ...
    else if (headerEnd === -1)
    {
      ...
      log.error('parseMessage() | malformed message');
    ...
    if (parsed !== true)
    {
      ...
      log.error(`parseMessage() | ${parsed.error}`);
