...
async function fetchAsset(url, timeoutMs = DEFAULT_FETCH_TIMEOUT_MS)
{
  ...
  try
  {
    ...
    const bytes = await Promise.race([ response.arrayBuffer(), trackedTimeoutPromise ]);
 ...
 * MediaStream 图的搭建由 `AiNSEngine` 负责。
 */
module.exports = class AiNSWorkletRuntime
{
  ...
  async createNode(audioContext)
  {
    ...
    logger.debug(`createNode() start: sampleRate=${audioContext && audioContext.sampleRate}`);
    try
    {
      ...
      this.workletNode = new AudioWorkletNode(audioContext, 'ai-noise-suppression-audio-processor', {
        ...
        processorOptions      : {
          ...
          suppressionLevel : this.config.level
        }
      });
    }
    catch (error)
    ...
    this.workletNode.port.onmessage = (event) =>
    {
      ...
      if (!data)
      {
        logger.debug('createNode() port message ignored: empty payload');

        return;
      }

      logger.debug(`createNode() port message: ${JSON.stringify(data)}`);
    ...
    this.workletNode.onprocessorerror = (event) =>
    {
      ...
      {
        try { this._onRuntimeFailure(event && event.message ? event.message : 'AudioWorklet processor error'); }
        catch (error) { logger.warn(`Runtime failure callback failed: ${getErrorMessage(error)}`); }
      }
    };

    logger.debug('createNode() complete');
  ...
  setLevel(level)
  {
    const clampedLevel = AiNSConfig.normalizeLevel(level, this.config.level);

    this.config.level = clampedLevel;

    logger.debug(`setLevel(): ${clampedLevel}`);

    if (!this.workletNode)
    {
      logger.debug('setLevel() deferred: worklet node not ready');
  ...
  destroy()
  {
    ...
    if (this.workletNode)
    {
      ...
      try { this.workletNode.disconnect(); }
      catch (error) { logger.warn(`destroy() disconnect failed: ${getErrorMessage(error)}`); }
      this.workletNode = null;
    }
