...
function collectCapabilityReport(engine)
{
  ...
  return {
    ...
    runtime      : engine ? {
      ...
      keepOtherTracks : Boolean(engine.keepOtherTracks),
      sampleRate      : engine.config && engine.config.sampleRate,
      level           : engine.config && engine.config.level,
...
class AiNSEngine
{
   ...
   * @param {boolean} [options.keepOtherTracks=true]
   * @param {number} [options.sampleRate=48000]
   * @param {number} [options.level=80] - 降噪强度 (0-100)
  ...
  constructor(options = {})
  {
    ...
    this.keepOtherTracks = normalizedOptions.keepOtherTracks;
  ...
  replaceTrack(input)
  {
    ...
    return this._enqueueGraphOperation(async() =>
    {
      ...
      const nextStream = this._buildAudioStream(nextAudioTrack);
      ...
      if (!this.processedStream)
      {
        throw new Error('AiNoiseSuppression.replaceTrack: failed to create processed MediaStream');
      }
  ...
  setLevel(level)
  {
    const nextLevel = AiNSConfig.normalizeLevel(level, this.config.level);

    this.config.level = nextLevel;
    logger.debug(`setLevel(): level=${nextLevel}`);
    this._workletRuntime.setLevel(nextLevel);
  }
  ...
  async ensureGraph()
  {
    ...
    if (!this.workletNode)
    {
      this.workletNode = await this._workletRuntime.createNode(this.audioContext);
    }
    ...
    logger.debug(`ensureGraph() complete: contextState=${this.audioContext.state} keepOtherTracks=${this.keepOtherTracks}`);
  }
  ...
  rebuildStream()
  {
    ...
    if (this.keepOtherTracks && this.originalStream)
    {
      this.originalStream.getTracks().forEach((track) =>
      {
        if (track.kind !== 'audio') outputTracks.push(track);
    ...
    logger.debug(`rebuildStream(): trackCount=${outputTracks.length}`);
  }
