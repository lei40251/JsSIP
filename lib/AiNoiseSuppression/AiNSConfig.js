...
const logger = new Logger('AiNSConfig');
 ...
 * @param {boolean} [options.keepOtherTracks=true] — 处理时是否保留原始流中非音频轨道（如视频轨）
 ...
 * @returns {boolean} returns.keepOtherTracks
 * @returns {number} returns.sampleRate
 * @returns {number} returns.level — 已钳位到 [0, 100]
...
exports.create = function(options)
{
  ...
  const config = {
    ...
    keepOtherTracks : exports.normalizeBoolean(options.keepOtherTracks, true),
    sampleRate      : exports.normalizePositiveInteger(options.sampleRate, DEFAULT_SAMPLE_RATE),
    level           : exports.normalizeLevel(options.level, DEFAULT_LEVEL),
 ...
 */
exports.normalizeLevel = function(value, fallback)
{
  const numberValue = Number(value);
  ...
  if (value !== undefined)
  {
    logger.debug(`normalizeLevel fallback: value=${value} fallback=${fallback}`);
  }
