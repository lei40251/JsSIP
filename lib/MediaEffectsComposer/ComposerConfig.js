...
const logger = new Logger('ComposerConfig');
 ...
 * @returns {boolean} returns.dropBusyFrames - 忙时是否丢帧
 * @returns {number} returns.maxFrameQueue - 最大帧队列长度
 * @returns {boolean} returns.keepDrawingBuffer - 是否保留绘图缓冲
...
exports.create = function(options)
{
  ...
  const config = {
    ...
    fps    : exports.normalizePositiveInteger(options.fps,
      Utils.shouldRecoverVideoFrameRateByUA() ? 60 : 15),
...
exports.normalizeSourceOptions = function(optionsOrSlot, index, defaultGain)
{
  ...
  else if (optionsOrSlot && typeof optionsOrSlot === 'object')
  {
    ...
    if (Object.prototype.hasOwnProperty.call(optionsOrSlot, 'aiBackground'))
    {
      options.aiBackground = optionsOrSlot.aiBackground;
    }
  }
