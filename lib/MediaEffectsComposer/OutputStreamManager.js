...
class OutputStreamManager
{
  ...
  _reportIssue(issue)
  {
    ...
    try { this._onIssue(Object.assign({ component: 'OutputStreamManager', severity: 'warn' }, issue)); }
    catch (e) { if (this._logger) this._logger.warn('OutputStreamManager issue callback failed: ' + (e.message || String(e))); }
  }
  ...
  async _writePresentedFrame(frameCtx)
  {
    ...
    const videoFrame = await this._createVideoFrameForFrame(frameCtx);
