...
class MediaEffectsComposer
{
   ...
   * @param {boolean} [options.dropBusyFrames=true]
  ...
  constructor(videos = [], options = {})
  {
    ...
    this._onOutputTrack = typeof options.onOutputVideoTrackChanged === 'function' ? options.onOutputVideoTrackChanged : null;
    this._issues = [];
    ...
    const config = ComposerConfig.create(options);
    ...
    this._config.aiVBManager = this._sourceAiVBManager;
    this._config.hasAiBackground = false;
    ...
    this._sourceRegistry = new Sources({
      ...
      normalizeGain      : ComposerConfig.normalizeGain,
      createVideoElement : this._streamToVideo.bind(this),
    ...
    this._outMgr = new OutputStream({
      canvas              : this._canvas,
      config              : this._config,
      createSinkVideo     : (stream) => this._createSinkVideo(stream),
      disposeSinkVideo    : (video) => this._disposeSinkVideo(video),
      logger              : logger,
      onIssue             : this._recordIssue.bind(this),
      onVideoTrackChanged : (change) =>
      {
        if (this._onOutputTrack)
        {
          return this._onOutputTrack(change);
        }
    ...
    this._renderLoop = new RenderLoop({
      ...
      syncSourceAudio     : () => this._syncExternalSourceAudio(),
      onStateChange       : () => {},
    ...
    this._layoutEngine = new LayoutEngine({
      ...
      createWatermarkItems : (payload) => this._createWmItems(payload),
    ...
    this._lastRenderLogSig = '';

    // -- 将初始传入的源加入混流 --
    this.addSource(videos, this._normalizeInitSources(options, videos.length));
  ...
  _forceFirstSlotFrameRate()
  {
    ...
    const fpsConstraints = constraints.frameRate;
    const frameRate = typeof fpsConstraints === 'number'
      ? fpsConstraints
      : fpsConstraints && (
        fpsConstraints.exact ||
        fpsConstraints.ideal ||
        fpsConstraints.max ||
        fpsConstraints.min
      );
   ...
   *
   * 同时将 aiBackground 配置透传给 AiVBState 做二级归一化。
   *
  ...
  {
    const normalized = ComposerConfig.normalizeSourceOptions(optionsOrSlot, index, this._config.audioGain);

    if (Object.prototype.hasOwnProperty.call(normalized, 'aiBackground'))
    {
      normalized.aiBackground = this._sourceAiVBManager.normalizeInput(normalized.aiBackground);
    }
   ...
   */
  _normalizeInitSources(options, sourceCount)
  {
  ...
  {
    const hasAiBackground = this._hasAiVBSource();
    const prevAiVBPolicy = this._config.hasAiBackground;

    this._config.hasAiBackground = hasAiBackground;

    if (prevAiVBPolicy !== hasAiBackground)
    {
      logger.debug(`Effect renderer policy updated: hasAiBackground=${hasAiBackground}`);
    }
  }
   ...
   */
  _fallbackMain2D(reason)
  {
    logger.warn(`Fallback to main-2d requested: ${reason}`);
    const fallbacked = this._renderLoop.fallbackMain2D(reason);
   ...
   */
  _fallbackMain(reason)
  {
    logger.warn(`Fallback to main-thread renderer requested: ${reason}`);

    return this._renderLoop.fallbackMain(reason);
  }
   ...
   * @returns {boolean} true=降级成功
   */
  _fallbackWorker2D(reason)
  {
    logger.warn(`Fallback to worker-2d renderer requested: ${reason}`);

    return this._renderLoop.fallbackWorker2D(reason);
  }
   ...
   */
  _createSinkVideo(mediaStream)
  {
    const video = this._streamToVideo(mediaStream);
  ...
  {
    this._audioComposer.syncSourceAudio();
  }
  ...
  _ensureMixedAudio(audioStream)
  {
    ...
    this._outMgr.ensureMixedAudio(audioStream || (this._audioDestination && this._audioDestination.stream));
  }
  ...
  _addAudioTracksToStream(targetStream, audioStream)
  {
    ...
    this._outMgr.addAudioTracks(targetStream, audioStream);
  }
  ...
  _getLegacyMirror(slot)
  {
    ...
    const normalizedSlot = ComposerConfig.normalizeSlot(slot, 0);
  ...
  _collectRenderInfo(logResult = true)
  {
    ...
    if (logResult)
    {
      const signature = [
        ...
        mergedInfo.frameControlMode || '',
        mergedInfo.fps || 0,
        mergedInfo.width || 0,
        mergedInfo.height || 0
      ].join('|');

      if (signature !== this._lastRenderLogSig)
      {
        this._lastRenderLogSig = signature;
        logger.debug(
          ...
          `insertableSupported=${Boolean(mergedInfo.insertableSupported)} generator=${mergedInfo.generatorType || '-'} ` +
          `insertableReason=${mergedInfo.insertableReason || '-'} ` +
          `captureFrameControl=${mergedInfo.frameControlMode || '-'} ` +
  ...
  _getConfigState()
  {
    const mirrorOverrides = this._slotMirrorOv || {};

    return {
      outputMirror          : Boolean(this._config.outputMirrorX),
      sourceMirror          : Boolean(this._config.mirrorX),
      sourceMirrorOverrides : Object.keys(mirrorOverrides).reduce((snapshot, key) =>
      {
        snapshot[key] = mirrorOverrides[key];

        return snapshot;
      }, {}),
      mirrorWatermarks : this._config.mirrorWatermarks !== false,
  ...
  async _getMixedOutput()
  {
    ...
    const videoStream = this._getVideoOutputSync();
    const mixedStream = this._outMgr.mixedStream;
    const hasLiveMixedVideo = mixedStream && mixedStream.getVideoTracks &&
      mixedStream.getVideoTracks().some((track) => track && track.readyState === 'live');
    const mixedVideoStream = hasLiveMixedVideo ? mixedStream : new MediaStream(videoStream.getVideoTracks());
  ...
  stop()
  {
    ...
    const cleanupSteps = [
      { name: 'render loop stop', run: () => this._renderLoop.stop() },
      { name: 'source removal', run: () => this._removeSourcesInternal(undefined) },
      { name: 'audio mixer stop', run: () => this._audioComposer.stop() },
      { name: 'AiVB clear', run: () => this._sourceAiVBManager.clear() },
      { name: 'layout cache clear', run: () => this._layoutEngine.clearAudioCache() },
      { name: 'renderer destroy', run: () => this._renderLoop.destroy() },
      { name: 'output stream stop', run: () => this._outMgr.stop() }
    ];

    cleanupSteps.forEach((step) =>
    {
      try { step.run(); }
      catch (error) { logger.warn(`Composer ${step.name} failed: ${error.message || String(error)}`); }
    });
  ...
  addSource(videos, optionsOrSlot)
  {
    ...
    const maxSources = ComposerConfig.getMaxSources();
    let appended = false;
    ...
    videos.forEach((video, index) =>
    {
      ...
      const replaceSlot = typeof sourceOptions.slot === 'number' &&
        this._sources.some((source) => source.slot === sourceOptions.slot);

      if (!replaceSlot && this._sources.length >= maxSources)
      {
  ...
  removeSource(target)
  {
    this._assertActive('removeSource()');
  ...
  clearSources()
  {
    this._assertActive('clearSources()');
   ...
   * 支持的 patch 字段：
   *   - outputMirror: 全局输出镜像
   *   - mirrorWatermarks: 输出镜像时水印是否同步镜像
  ...
  async setConfig(patch)
  {
    ...
    if (patch.sourceMirrorOverrides && typeof patch.sourceMirrorOverrides === 'object')
    {
      ...
      {
        const normalizedSlot = ComposerConfig.normalizeSlot(slotKey, 0);
    ...
    if (refreshMirrorPolicy)
    {
      if (preloadAiVBAssets)
      {
        this._preloadAiVB();
      }
      this._refreshFxRenderPolicy();
    ...
    return this._getConfigState();
  }

  getState()
  {
    this._assertActive('getState()');

    return {
      sources : this._sourceRegistry.getSnapshot(),
      config  : this._getConfigState(),
  ...
  {
    const request = this._normalizeOutput(options);
    const type = request.type;

    logger.debug(`getOutput(): ${JSON.stringify(request)}`);
    this._assertActive('getOutput()');
  ...
  releaseOutput(options)
  {
    ...
    this._assertActive('releaseOutput()');
    ...
    return this._audioComposer.releaseSubmixStream(request);
  }
  ...
  {
    this._assertActive('setAiBackground()');
  ...
  {
    this._assertActive('getAiBackground()');
  ...
  {
    this._assertActive('clearAiBackground()');
  ...
  {
    this._assertActive('setWatermarkMirror()');

    return this.setConfig({ mirrorWatermarks: enabled });
  }

  getWatermarkMirror()
  {
    return this.getState().config.mirrorWatermarks;
  }
  ...
  {
    this._assertActive('setSourceMirror()');
  ...
  {
    this._assertActive('getSourceMirror()');

    return this._getLegacyMirror(slot);
  }
  ...
  {
    this._assertActive('clearSourceMirror()');
   ...
   * 用于 setAiBackground() / getAiBackground() 等效果设置方法。
   *
   * @param {number|MediaStream|string} slotOrTarget - slot 编号或 Stream 对象
  ...
  getRenderInfo()
  {
    this._assertActive('getRenderInfo()');
  ...
  getAudioInfo()
  {
    this._assertActive('getAudioInfo()');
  ...
  getCapabilities()
  {
    this._assertActive('getCapabilities()');

    const aiBackgroundSupported = Boolean(
      typeof document !== 'undefined' && document.createElement &&
      typeof WebAssembly !== 'undefined'
    );
    const aiBackgroundEnabled = Boolean(this._config.hasAiBackground);

    return {
      limits : {
        maxSources : ComposerConfig.getMaxSources()
      },
      features : {
        multiSource           : ComposerConfig.getMaxSources() > 1,
        aiBackground          : aiBackgroundEnabled,
        aiBackgroundSupported : aiBackgroundSupported,
        aiBackgroundEnabled   : aiBackgroundEnabled,
        outputMirror          : true,
        audioSubmix           : true,
        insertableConfigured  : Boolean(this._config.insertable)
      },
  ...
  {
    this._assertActive('clearWatermarks()');
  ...
  getVideoStream()
  {
    ...
    this._assertActive('getVideoStream()');
  ...
  async getAudioStream(options)
  {
    ...
    this._assertActive('getAudioStream()');
   ...
   */
  async getSubmixStream(options)
  {
    logger.debug(`getSubmixStream(): ${JSON.stringify(options || null)}`);
    ...
    logger.debug(`getSubmixStream() resolved: tracks=${audioStream ? audioStream.getAudioTracks().length : 0}`);
   ...
   */
  releaseSubmixStream(options)
  {
    logger.debug(`releaseSubmixStream(): ${JSON.stringify(options || null)}`);
    const released = this.releaseOutput(Object.assign({ type: 'audio' }, options instanceof Array ? { slots: options } : (options || {})));

    logger.debug(`releaseSubmixStream() complete: released=${released}`);
