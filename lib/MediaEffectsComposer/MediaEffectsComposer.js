...
class MediaEffectsComposer
{
  ...
  async _getMixedOutput()
  {
    ...
    const existingMixedStream = this._outMgr.mixedStream;
    const hasLiveMixedVideo = existingMixedStream && existingMixedStream.getVideoTracks &&
      existingMixedStream.getVideoTracks().some((track) => track && track.readyState === 'live');
    const mixedVideoStream = hasLiveMixedVideo ? existingMixedStream : new MediaStream(videoStream.getVideoTracks());
  ...
  stop()
  {
    ...
    const cleanupSteps = [
      ...
      { name: 'layout cache clear', run: () => this._layoutEngine.clearAudioPlaceholderCache() },
  ...
  addSource(videos, optionsOrSlot)
  {
    ...
    videos.forEach((video, index) =>
    {
      ...
      const replacesExistingSlot = typeof sourceOptions.slot === 'number' &&
  ...
  getCapabilityReport()
  {
    ...
    const sourceAiVirtualBackgroundSupported = Boolean(
      typeof document !== 'undefined' && document.createElement &&
      typeof WebAssembly !== 'undefined'
    );
    const sourceAiVirtualBackgroundEnabled = Boolean(this._config.hasSourceAiVirtualBackground);
