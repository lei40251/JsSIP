...
//
// AI 虚拟背景路径：当 item 带有 aiBackground 配置且 runtimeEnabled=true 时，
...
var mirrorWatermarks = true; // 输出镜像时水印是否一起镜像
...
function hasAiVirtualBackground(item)
{
  return Boolean(item &&
    item.aiBackground &&
    item.aiBackground.enabled !== false &&
    item.aiBackground.mode &&
    item.aiBackground.mode !== 'none');
}
...
async function ensureBackgroundImage(url)
{
  ...
  {
    backgroundState.promise = fetchBackgroundBitmap(url)
...
async function getRenderableSurface(item)
{
  ...
  {
    return { surface: item.frame, sourceMirrorApplied: false };
  }

  var sourceState = resolveAiVBState(item.id, item.aiBackground);
  ...
  foregroundSurface.context.filter = buildForegroundFilter(item.aiBackground.postProcessing);
  ...
  if (item.aiBackground.mode === 'blur')
  {
    var postProcessing = item.aiBackground.postProcessing || {};
  ...
  else if (item.aiBackground.mode === 'image')
  {
    var backgroundImage = await ensureBackgroundImage(item.aiBackground.imageUrl || '');
  ...
  else if (item.aiBackground.mode === 'color')
  {
    outputSurface.context.fillStyle = item.aiBackground.backgroundColor || '#00ff00';
  ...
  return { surface: outputSurface.canvas, sourceMirrorApplied: true };
}
...
async function render(payload)
{
  ...
  try
  {
    ...
    mirrorWatermarks = payload.mirrorWatermarks !== false;
...
async function renderWebGL2(payload)
{
  ...
  drawWatermarksWebGL2(payload.outputWatermarks || [], mirrorWatermarks ? outputMirrorX : false);
...
async function renderCanvas2D(payload)
{
  ...
  for (var index = 0; index < items.length; index += 1)
  {
    ...
    drawSurface(ctx, surface, draw.x, draw.y, draw.width, draw.height,
      (renderable.sourceMirrorApplied ? false : Boolean(item.mirrorX)) !== outputMirrorX, false);
  }

  drawWatermarksCanvas2D(payload.sourceWatermarks || [], outputMirrorX);
  drawWatermarksCanvas2D(payload.outputWatermarks || [], mirrorWatermarks ? outputMirrorX : false);
}
...
{
  var workerSource = workerMain.toString();
  var helperFactorySource = SegmentationCommon.getWorkerSegmentationHelpersFactorySource();
