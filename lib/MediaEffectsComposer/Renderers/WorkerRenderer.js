...
module.exports = class WorkerRenderer 
{
   ...
   * @param {boolean} [config.dropBusyFrames=true] - Worker 忙时是否丢弃新帧
  ...
  constructor(config, info)
  {
    ...
    this._preferDirectFrameSource = Boolean(this._config && this._config.insertable);
    ...
    if (data.type === 'rendered')
    {
      ...
      const canUseDirectFrameSource = this._preferDirectFrameSource &&
        !(
          ...
          this._config &&
          this._config.hasAiBackground === true
        );
      ...
      if (data.bitmap && this._outputContext)
      {
        ...
        this._drawWorkerBitmap(data.bitmap);
  ...
  render(payload)
  {
    ...
    if (!this._workerReady)
    {
      ...
      if (this._config.dropBusyFrames)
      {
        this._info.droppedFrames += 1;
  ...
  async _renderInWorker(payload)
  {
    ...
    try
    {
      result = await this._createWorkerData(payload);
   ...
   */
  async _createWorkerData(payload)
  {
    const items = [];
    ...
    try
    {
      for (let idx = 0; idx < payload.items.length; ++idx)
      {
        ...
        items.push({
          ...
          aiBackground : this._normalizeWorkerAiVB(item.aiBackground),
          frame
        });
        transfers.push(frame);
      }

      await this._appendWm(payload.sourceWatermarks, sourceWatermarks, transfers);
      await this._appendWm(payload.outputWatermarks, outputWatermarks, transfers);
    }
    catch (error)
    {
   ...
   */
  async _appendWm(watermarks, target, transfers)
  {
