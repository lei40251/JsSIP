...
module.exports = class MainWebGL2Renderer 
{
  ...
  render(payload)
  {
    ...
    payload.items.forEach((item) =>
    {
      ...
      const drawOutputMirrorX = surface && surface.aiVirtualBackgroundComposed ? false : payload.outputMirrorX;
  ...
  _resolveItemSurface(item, outputMirrorX, canvasWidth)
  {
    ...
    const effect = this._aiVirtualBackgroundManager.getRenderableState(item.source, item.video);
    ...
    const effectiveMirrorX = Boolean(item.mirrorX) !== Boolean(outputMirrorX);
    const composedSurface = this._composeAiVirtualBackgroundSurface(item, effect, draw, effectiveMirrorX);
    ...
    return {
      ...
      aiVirtualBackgroundComposed : true,
  ...
  _composeAiVirtualBackgroundSurface(item, effect, draw, mirrorX)
  {
    ...
    foregroundSurface.context.filter = buildForegroundEnhancementFilter(effect.config.postProcessing);
    ...
    else if (effect.config.mode === 'image')
    {
      ...
      {
        this._drawCoverSurfaceToContext(outputSurface.context, effect.backgroundImage, {
          x      : 0,
          y      : 0,
          width  : outputSurface.canvas.width,
          height : outputSurface.canvas.height
