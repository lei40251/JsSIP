...
class OutputStream
{
  ...
  constructor(options)
  {
    ...
    this._insertableByCfg = Boolean(this._config && this._config.insertable);
  ...
  getVideoStream(drawFirstFrame)
  {
    ...
    return this._createCaptureVideo();
  }

  _createCaptureVideo()
  {
    const videoStream = new MediaStream();
    const capturedStream = this._createCaptureStream();
    ...
    this._insertableActive = false;
    this._configFrameCtrl(capturedStream);
    this._ensureCaptureSink(capturedStream);
  ...
  {
    if (this._config.manualFrameControl !== false)
    {
  ...
  _configFrameCtrl(capturedStream)
  {
    this._capturedVideoTrack = null;
    this._manualFrameControl = false;
    ...
    this._manualFrameControl = Boolean(
      this._config.manualFrameControl !== false &&
      videoTrack &&
      typeof videoTrack.requestFrame === 'function'
    );
  }

  _ensureCaptureSink(capturedStream)
  {
  ...
  _requestCaptureFrame()
  {
    ...
    {
      const fallbackApplied = this._fallbackAutoCapture(error);
  ...
  _fallbackAutoCapture(error)
  {
    ...
    this._manualFrameControl = false;
    this._ensureCaptureSink(capturedStream);
    this._replaceVideoTrack(previousTrack, nextTrack, 'capture-stream-auto-fallback');
  ...
  _createInsertableStream()
  {
    ...
    catch (error)
    {
      ...
      this._reportIssue({
        ...
        message : getErrorMessage(error),
        details : {
          insertableConfigured : Boolean(this._insertableByCfg),
      ...
      this._teardownInsertable(false);
  ...
  onFramePresented(frameCtx)
  {
    ...
    if (this._pendingWrite)
    {
      ...
      if (replacedFrame)
      {
        this._closeUsedFrame(replacedFrame);
      }
    ...
    this._writeFrame(frameCtx)
      .then(() =>
      {
        this._pendingWrite = false;
        this._flushPendingFrame();
      })
      .catch((error) =>
      {
        this._pendingWrite = false;
        this._handleInsertError(error);
        this._flushPendingFrame();
      });
  }
  ...
  async _writeFrame(frameCtx)
  {
    ...
    try
    {
      videoFrame = await this._makeVideoFrame(frameCtx);
  ...
  async _makeVideoFrame(frameCtx)
  {
  ...
  _handleInsertError(error)
  {
    ...
    if (this._writeFailCount >= this._maxWriteFails)
    {
      ...
      const fallbackApplied = this._fallbackToCapture();
  ...
  _fallbackToCapture()
  {
    ...
    try
    {
      capturedStream = this._createCaptureStream();
    }
    catch (error)
    {
      capturedStream = null;
    ...
    this._configFrameCtrl(capturedStream);
    this._ensureCaptureSink(capturedStream);
    this._replaceVideoTrack(previousTrack, nextTrack, 'insertable-write-fallback');
    this._teardownInsertable(true);
  ...
  _replaceVideoTrack(previousTrack, nextTrack, reason)
  {
  ...
  setCanvas(canvas)
  {
    ...
    this._configFrameCtrl(capturedStream);
    this._ensureCaptureSink(capturedStream);
    this._replaceVideoTrack(previousTrack, nextTrack, 'renderer-canvas-replaced');
   ...
   */
  addAudioTracks(targetStream, audioStream)
  {
  ...
  _teardownInsertable(stopTrack)
  {
    ...
    if (pendingFrame)
    {
      this._closeUsedFrame(pendingFrame);
    }
  ...
  getOutputRouteInfo()
  {
    return {
      ...
      frameControlMode     : this._manualFrameControl ? 'manual-request-frame' : 'auto-capture-fps',
      insertableActive     : Boolean(this._insertableActive),
      insertableConfigured : Boolean(this._insertableByCfg),
      insertableSupported  : Boolean(this._insertableSupport && this._insertableSupport.supported),
      generatorType        : (this._insertableSupport && this._insertableSupport.generatorType) || '',
      insertableReason     : (this._insertableSupport && this._insertableSupport.reason) || '',
      writeFailures        : this._writeFailCount || 0,
      hasGeneratorTrack    : Boolean(this._generatorTrack),
      hasCaptureStream     : Boolean(this._capturedStream),
      captureSinkActive    : Boolean(this._captureSinkVideo)
    };
  }
