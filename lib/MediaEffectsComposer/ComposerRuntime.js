...
const SourceAiVBController = require('./aiVirtualBackground/SourceAiVBController');
...
class ComposerState
{
  ...
  {
    const info = this._getRenderInfo();
    const outputRouteInfo = this._getOutputRouteInfo();
  ...
  getConfigSnapshot()
  {
    ...
    const slotMirrorXOverrides = data.slotMirrorXOverrides || {};
...
class ComposerRuntime
{
  ...
  constructor(videos = [], options = {})
  {
    ...
    // -----------------------------------------------------------------------
    // 输入源存储（SourceStore）—— 统一管理所有视频/音频源的生命周期。
    //
    // 职责：
    //   1. 维护源列表，处理增删查改
    ...
    this._sourceRegistry = new SourceStore({
      logger             : logger,
   ...
   *   2. 保存为 mixedStream（供后续 _ensureMixedStreamAudioTrack 补充音频轨）
