...
function createRenderer(canvas, config, hooks)
{
  ...
  if (
    !forceMainThread &&
    (mode !== 'auto' || canUseWorkerRenderer(canvas)) &&
  ...
  {
    errors.push(error.message || String(error));
    const replacementCanvas = createReplacementCanvas(canvas);
    ...
    const renderer = createMain2D(replacementCanvas, config, true, errors.join('; '));
...
function createMainFallback(canvas, config, requestedMode, reason, hooks)
{
  ...
  catch (error)
  {
    ...
    const renderer = createMain2D(replacementCanvas, config, true, `${reason}; ${error.message || String(error)}`);
...
function canUseWorkerRenderer(canvas)
{
  return Boolean(
...
class RenderLoop
{
   ...
   * @param {Function} options.syncSourceAudio - 同步外部源音频的函数
  ...
  constructor(options)
  {
    ...
    this._syncExternalSourceAudio = options.syncSourceAudio;
  ...
  fallbackRenderer(reason)
  {
    ...
    {
      const mainWebGL2 = currentInfo.actualMode === 'main-webgl2' ? false :
        this._tryMainWebGL2(currentInfo, reason);
      ...
      const worker2D = this._tryWorker2D(currentInfo, reason);
    ...
    return this.fallbackMain2D(reason, currentInfo);
  }

  fallbackMain2D(reason, info)
  {
    if (this._logger)
    {
  ...
  fallbackMain(reason, info)
  {
    ...
    {
      const mainWebGL2 = this._tryMainWebGL2(currentInfo, reason);
  ...
  fallbackWorker2D(reason, info)
  {
    ...
    return this._tryWorker2D(currentInfo, reason);
  }

  _tryMainWebGL2(currentInfo, reason)
  {
    try
    {
  ...
  _tryWorker2D(currentInfo, reason)
  {
    try
    {
      ...
      this._activateRenderer(() => new WorkerRenderer(workerConfig, {
        ...
        {
          this.fallbackMain2D(fallbackReason || 'Worker Canvas2D renderer failed at runtime');
        }
      }), 'fallback-worker-2d');
  ...
  _commitCanvasChange(canvas)
  {
    ...
    {
      try { this._onCanvasChange(canvas); }
      catch (error)
      {
  ...
  {
    if (!renderer || !renderer.setFrameCallback)
    {
      return;
    }
