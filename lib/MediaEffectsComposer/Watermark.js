...
class Watermark
{
  ...
  _reportIssue(issue)
  {
    ...
    try { this._onIssue(Object.assign({ component: 'Watermark' }, issue)); }
    catch (e) { if (this._logger) this._logger.warn(`Watermark issue callback failed: ${ e.message || String(e)}`); }
  }
  ...
  {
    const list = this._normalizeWatermarkList(watermarks).slice(0, MAX_WATERMARKS);
    ...
    const loads = this._watermarks.map((watermark) => this._prepareWatermark(watermark, generation));
  ...
  _normalizeWatermark(input)
  {
    ...
    const fontSize = normalizeBoundedInteger(input.fontSize, 1, MAX_FONT_SIZE, DEFAULT_FONT_SIZE);
  ...
  _prepareImageWatermark(watermark, generation)
  {
    ...
    const imageWidth = Number(image.width || image.videoWidth || image.naturalWidth);
    const imageHeight = Number(image.height || image.videoHeight || image.naturalHeight);

    if (!Number.isFinite(imageWidth) || !Number.isFinite(imageHeight) || imageWidth <= 0 || imageHeight <= 0 ||
      imageWidth > MAX_SURFACE_DIMENSION || imageHeight > MAX_SURFACE_DIMENSION ||
      imageWidth * imageHeight > MAX_SURFACE_PIXELS)
    {
      watermark.status = 'error';
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      image.onload = () => settle(() =>
      {
        ...
        if (!width || !height || width > MAX_SURFACE_DIMENSION || height > MAX_SURFACE_DIMENSION ||
          width * height > MAX_SURFACE_PIXELS)
        {
  ...
  _createTextSurface(watermark)
  {
    ...
    let width = Math.min(MAX_SURFACE_DIMENSION, Math.max(1, Math.ceil(measured + (watermark.padding * 2))));
    let height = Math.min(MAX_SURFACE_DIMENSION, Math.max(1, Math.ceil(ascent + descent + (watermark.padding * 2))));

    if (width * height > MAX_SURFACE_PIXELS)
    {
      const scale = Math.sqrt(MAX_SURFACE_PIXELS / (width * height));
