...
module.exports = class SourceAiVBController
{
  ...
  _reportIssue(issue)
  {
    ...
    try { this._onIssue(Object.assign({ component: 'SourceAiVBController' }, issue)); }
    catch (e) { if (this._logger) this._logger.warn('SourceAiVBController issue callback failed: ' + (e.message || String(e))); }
  }
  ...
  _refreshQueuedSegInput(state, videoElement)
  {
    ...
    if (state.lastQueuedSegAt &&
      now - state.lastQueuedSegAt < minInterval)
    {
      return;
    }
