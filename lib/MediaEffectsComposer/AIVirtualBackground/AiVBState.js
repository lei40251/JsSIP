...
module.exports = class AiVBState
{
  ...
  _scheduleSegmentation(state, videoElement, now = this._now())
  {
    ...
    if (state.pendingSegmentation)
    {
      ...
      const segmentationInput = this._getSegmentationInput(state, videoElement, 'queued');
      const input = segmentationInput || videoElement;
      const queuedFrame = this._getSegmentationFrame(state, videoElement, 'queued');
    ...
    const input = segmentationInput || videoElement;
    const activeFrame = this._getSegmentationFrame(state, videoElement, 'active');
  ...
  _bindSegmentationPromise(state, promise, generation)
  {
    promise
      .then((result) =>
      {
        ...
        state.latestFrame = this._copyLatestSegFrame(state, state.segFrameMap.get(promise));
   ...
   */
  _getSegmentationFrame(state, videoElement, target = 'active')
  {
   ...
   */
  _getSegmentationInput(state, videoElement, target = 'active')
  {
    if (!state || !videoElement || typeof document === 'undefined')
    {
      return null;
