...
class AudioMixer
{
  ...
  _reportIssue(issue)
  {
    ...
    try { this._onIssue(Object.assign({ component: 'AudioMixer', severity: 'warn' }, issue)); }
    catch (e) { if (this._logger) this._logger.warn(`AudioMixer issue callback failed: ${ e.message || String(e)}`); }
  }
   ...
   */
  getSubmixStream(options)
  {
    if (this._logger)
    {
      this._logger.debug(`getSubmixStream(): ${JSON.stringify(options || null)}`);
    }
  ...
  releaseSubmixStream(options)
  {
    if (this._logger)
    {
      this._logger.debug(`releaseSubmixStream(): ${JSON.stringify(options || null)}`);
    }
    ...
    if (this._useIsolatedAudioCtx(options))
    {
      ...
      this._disconnectSubmix(submix, true);
  ...
  _runScheduledRefresh()
  {
    ...
    this._audioRefreshPromise = this._refreshAudioConns()
      .catch((error) =>
      {
  ...
  _destroySourceNode(source)
  {
    ...
    this._unbindTrackListeners(source);
  ...
  stop()
  {
    ...
    if (audioContext)
    {
      try
      {
        this._closeCtxPromise = Promise.resolve(audioContext.close())
          .catch((error) =>
          {
      ...
      {
        this._closeCtxPromise = Promise.resolve();
        this._logger.warn(`Failed to close AudioContext: ${error.message || String(error)}`);
      }
    }
    this._audioSources = new Set();
  ...
  _ensureAudioSystem(options)
  {
    ...
    if (!this._audioReadyPr)
    {
      ...
      const resumePromise = this._audioContext.state === 'suspended' ?
        Promise.resolve().then(() => this._audioContext.resume()) :
        Promise.resolve();
    ...
    return this._audioReadyPr.then((ready) =>
    {
      ...
      {
        this._audioDestination = this._createAudioDest();
      }
  ...
  _ensureBusDestination(bus)
  {
    ...
    if (!bus.destination)
    {
      bus.destination = this._createAudioDest();
    ...
    if (!this._isDestTrackLive(bus.destination))
    {
      ...
      if (ended)
      {
        bus.connections.forEach((connection) => this._disconnectBusConn(bus, connection));
  ...
  _refreshAudioConns()
  {
    ...
    this._isolatedSubmixes.forEach((submix) =>
    {
      if (submix.requested)
      {
        chain = chain.then(() => this._refreshSubmixConns(submix));
      }
    });
  ...
  _disconnectBusSource(bus, source)
  {
    ...
    this._disconnectBusConn(bus, connection);

    bus.connections.delete(source.id);
    this._updateTargetAudioInfo(bus);
  }

  _disconnectBusConn(bus, connection)
  {
    if (!connection)
    {
      return;
  ...
  _createSubmixAudio(request)
  {
    ...
    if (existing)
    {
      this._disconnectSubmix(existing, true);
      this._isolatedSubmixes.delete(request.key);
    }

    const submix = this._getSubmix(request.key, request.slots);

    submix.requested = true;
    this._isolatedContextRequests += 1;

    return this._refreshSubmixConns(submix);
  }

  _getSubmix(key, slots)
  {
  ...
  _disconnectSubmix(submix, closeContext)
  {
    if (!submix)
    {
      return;
  ...
  _refreshSubmixConns(submix)
  {
    ...
    return this._ensureSubmix(submix)
      .then((ready) =>
      {
        ...
        liveSources.forEach((source) =>
        {
          ...
          try
          {
            ...
            this._setCtxGain(gainNode, source.gain, submix.audioContext);
  ...
  _refreshBusAudio(bus, options)
  {
    ...
    return this._ensureAudioSystem({ defaultDestination: !bus })
      .then((ready) =>
      {
        ...
        if (this._getTargetConnCount(bus) === 0 && connectedSources.length === 0)
        {
  ...
  _connectSource(source, bus)
  {
    ...
    let gainNode = null;
    let createdAudioSource = false;
    let createdMasterGain = false;
    ...
    try
    {
      ...
      if (!source.audioSourceNode)
      {
        ...
        this._bindTrackListeners(source, signature);
      }
  ...
  { 
    Object.assign(this._audioInfo, {
      ...
      boundTrackListeners     : this._countTrackListeners(),
  ...
  _setGainValue(gainNode, value)
  {
    ...
    this._setCtxGain(gainNode, value, this._audioContext);
  }

  _setCtxGain(gainNode, value, audioContext)
  {
    if (!gainNode || !gainNode.gain)
    {
      return;
  ...
  _bindTrackListeners(source, signature)
  {
