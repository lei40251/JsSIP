...
class LayoutEngine
{
  ...
  constructor(options)
  {
    ...
    this._createWmItems = options.createWatermarkItems;
  ...
  createRenderPayload()
  {
      ...
      if (hasVideo)
      {
        ...
        if (draw)
        {
          items.push({
            ...
            source       : source,
            mirrorX      : mirrorX,
            aiBackground : source.aiBackground || null,
            draw         : draw
          });
        }
      }
    ...
    const watermarks = this._createWmItems ? this._createWmItems(payload) : null;
