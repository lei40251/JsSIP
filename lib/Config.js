...
const checks = {
  ...
  optional : {
    ...
    ice_checking_timeout(ice_checking_timeout)
    {
      if (Utils.isDecimal(ice_checking_timeout))
      {
        const value = Number(ice_checking_timeout);
    ...
    candidates_transport(candidates_transport)
    {
    ...
    {
      const _audio_payloads = [];

      if (Array.isArray(audio_payloads) && audio_payloads.length)
      {
        for (const payload of audio_payloads)
        {
    ...
    {
      const _video_payloads = [];

      if (Array.isArray(video_payloads) && video_payloads.length)
      {
        for (const payload of video_payloads)
        {
