...
module.exports = class getStats extends EventEmitter
{
  ...
  start()
  {
    this._statsTimer = setInterval(async() =>
    {
      ...
      try
      {
        micl = await Utils.getMicrophones().length;

        this._pc.getSenders().forEach((s) =>
        {
          const trackStatus = `micl: ${micl},id: ${s.track.id}, enabled: ${s.track.enabled}, label: ${s.track.label},kind: ${s.track.kind},muted: ${s.track.muted},readyState: ${s.track.readyState},transport: ${s.transport.state};`;

          logger.debug(`curr ${s.track.kind} track status: ${trackStatus} ***** settings: ${JSON.stringify(s.track.getSettings())} ***** constraints: ${JSON.stringify(s.track.getConstraints())} ***** capabilities: ${JSON.stringify(s.track.getCapabilities())}`);
        });
      }
      catch (error) { logger.error(`getMicrophones or others error ${JSON.stringify(error)}`); }
  ...
  parseReport(stats, inform)
  {
    ...
    this._networkQuality = {
      ...
      // eslint-disable-next-line max-len
      uplinkLoss             : uplinkLoss.length > 0 ? Math.floor(uplinkLoss.reduce((pre, cur) => pre + cur)/uplinkLoss.length)||null : null,
      // eslint-disable-next-line max-len
      ...
      // eslint-disable-next-line max-len
      downlinkLoss           : downlinkLoss.length > 0 ? Math.floor(downlinkLoss.reduce((pre, cur) => pre + cur)/downlinkLoss.length)||null : null
    };
