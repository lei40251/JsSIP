  ...
  _parseReceiverReport(reports, receiver, shared, inform)
  {
    ...
    reports.forEach((report) =>
    {
      ...
      switch (report.type)
      {
        ...
        {
          // 用于计算速率和丢包的值
          calc_bytesReceived =report['bytesReceived'] > previewStats.bytesReceived ? report['bytesReceived'] - (previewStats.bytesReceived || 0) : 0;
          calc_packetsReceived = report['packetsReceived'] > previewStats.packetsReceived ? report['packetsReceived'] - (previewStats.packetsReceived || 0) : 0;
          calc_packetsLost = report['packetsLost'] > previewStats.packetsLost ? report['packetsLost'] - (previewStats.packetsLost || 0) : 0;
    ...
    calc_packetsReceived === 0 && (tmpObject = Object.assign({}, tmpObject, { packetsReceived: tmpObject['packetsReceived'], calc_speed: 0, calc_loss: 100, calc_bytesReceived: 0 }));
    // 合并统计结果
  ...
  _createReport()
  {
    ...
    function formatStats(oldStats)
    {
      ...
      newStats.upStreams.push({
        ...
        speed       : oldUpStreams['audio'].calc_speed === 0 ? 0 : (oldUpStreams['audio'].calc_speed / 1000).toFixed(1)
      });
      ...
      newStats.downStreams.push({
        ...
        speed           : oldDownStreams['audio'].calc_speed === 0 ? 0: (oldDownStreams['audio'].calc_speed / 1000).toFixed(1)
      });
      ...
      for (const type of [ 'video', 'shared' ])
      {
        if (Object.keys(oldUpStreams[type]).length > 1)
        {
          ...
          newStats.upStreams.push({
            ...
            speed           : oldUpStreams[type].calc_speed === 0 ? 0 : (oldUpStreams[type].calc_speed / 1000).toFixed(1)
          });
        }
        ...
        if (Object.keys(oldDownStreams[type]).length > 1)
        {
          ...
          newStats.downStreams.push({
            ...
            speed           : oldDownStreams[type].calc_speed === 0 ? 0 : (oldDownStreams[type].calc_speed / 1000).toFixed(1)
          });
        }
      }
