      ...
      switch (report.type)
      {
        ...
        case 'outbound-rtp':
          ...
          if (report.kind === 'video')
          {
            ...
            const calc_packetsLost = this._remoteInboundRtps.get(report['remoteId']).packetsLost - (previewStats.packetsLost || 0);
            ...
            tmpObject['packetsLost'] = this._remoteInboundRtps.get(report['remoteId']).packetsLost;
            ...
            else if ('fractionLost' in this._remoteInboundRtps.get(report['remoteId']))
            {
            ...
            console.warn('vuloss: ', Math.floor(calc_packetsLost * 100 / calc_packetsSent), '#', Math.floor(this._remoteInboundRtps.get(report['remoteId']).fractionLost * 100));
          ...
          else if (report.kind === 'audio')
          {
            ...
            tmpObject['packetsSent'] = report['packetsSent'];
            // 通过 remote-inbound-rtp 报告获取的原始值
            tmpObject['sendPacketsLost'] = this._remoteInboundRtps.get(report['remoteId']).packetsLost;
            ...
            console.warn('auloss: ', Math.floor(calc_packetsLost * 100 / calc_packetsSent), '#', Math.floor(this._remoteInboundRtps.get(report['remoteId']).fractionLost * 100));
