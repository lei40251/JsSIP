...
module.exports = class getStats extends EventEmitter
{
  ...
  _parseReport(stats, inform)
  {
    ...
    stats.forEach((report) =>
    {
      ...
      {
        this._mediaSourceIdToTrackIdentifier.set(report.id, report.trackIdentifier);
      }
    ...
    this._pc.getTransceivers().forEach((transceiver) =>
    {
      ...
      {
        this._remoteSharedTrackIdentifier = transceiver.receiver.track.id;
      }
      ...
      {
        this._localSharedTrackIdentifier = transceiver.sender.track.id;
      }
    });
    ...
    stats.forEach((report) =>
    {
      ...
      switch (report.type)
      {
        case 'candidate-pair':
          ...
          {
            (report['currentRoundTripTime']) && (this._newStats.transport.RTT = Math.floor(1e3 * report['currentRoundTripTime']));
          }
          break;
        case 'outbound-rtp':
          // 上行视频
          if (report.kind === 'video')
          {
            const tmpObject = {};
            // 当前统计报告的类型
            const type = this._mediaSourceIdToTrackIdentifier.get(report['mediaSourceId']) === this._localSharedTrackIdentifier ? 'shared' : 'camera';

            console.warn('tid: ', this._localSharedTrackIdentifier, report);
            // 前一次的统计结果
            ...
            else if (this._remoteInboundRtps.get(report['remoteId']) && ('fractionLost' in this._remoteInboundRtps.get(report['remoteId'])))
            {
              ...
              if (this._remoteInboundRtps.get(report['remoteId']).roundTripTime)
              {
                this._newStats.transport.RTT = Math.floor(1e3 * this._remoteInboundRtps.get(report['remoteId']).roundTripTime);
              }
            }
            else
            {
      ...
      for (const type of [ 'camera', 'shared' ])
      {
        if (oldUpStreams[type])
        {
          ...
          newStats.video.upStreams.push({
            ...
            speed           : `${(oldUpStreams[type].calc_speed / 1000).toFixed(1)}kbps`
          });
        }
      }
      ...
      for (const type of [ 'camera', 'shared' ])
      {
        if (oldDownStreams[type])
        {
          ...
          newStats.video.downStreams.push({
            ...
            speed           : `${(oldDownStreams[type].calc_speed / 1000).toFixed(1)}kbps`
          });
        }
      }
