...
const debug = require('debug')('FlyInn:sanityCheck');
...
function rfc3261_8_1_3_3()
{
  if (message.getHeaders('via').length > 1)
  {
    ...
    log.debug('more than one Via header field present in the response, dropping the response');
...
function rfc3261_18_3_response()
{
  ...
  if (len < contentLength)
  {
    ...
    log.debug('message body length is lower than the value in Content-Length header field, dropping the response');
...
function minimumHeaders()
{
  ...
  for (const header of mandatoryHeaders)
  {
    if (!message.hasHeader(header))
    {
      ...
      log.debug(`missing mandatory header field : ${header}, dropping the response`);
