...
function rfc3261_16_3_4()
{
  ...
  const transactions = message.method === CRTC_C.INVITE ?
    ua._transactions.ict :
    ua._transactions.nict;

  for (const transaction in transactions)
  {
    if (Object.prototype.hasOwnProperty.call(transactions, transaction))
    {
      const tr = transactions[transaction];

      if (
        tr.request.call_id === message.call_id &&
        tr.request.from_tag === message.from_tag &&
        tr.request.cseq === message.cseq
      )
      {
        reply(482);
...
function reply(status_code)
{
  ...
  let to;
  let response = `SIP/2.0 ${status_code} ${CRTC_C.REASON_PHRASE[status_code]}\r\n`;
