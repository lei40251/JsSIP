...
const socket = new CRTC.WebSocketInterface('wss://pro.vsbc.com:60040/wss');
  ...
  e.session.on('mode', function (d) {
    ...
    if (RTCPeerConnection.prototype.getSenders) {
      e.session.connection.getSenders().forEach((sender) => {
        ...
        if ((sender.track && sender.track.kind === 'video' && sender.track.readyState === 'live') && (d.mode === 'audio' && sender.track.kind !== 'video')) {
    ...
    if (RTCPeerConnection.prototype.getReceivers) {
      e.session.connection.getReceivers().forEach((receiver) => {
        if ((receiver.track && receiver.track.readyState === 'live') && (d.mode === 'audio' && receiver.track.kind !== 'video')) {
