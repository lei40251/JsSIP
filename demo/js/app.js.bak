(function () {
  ...
  var client = null; // 本端媒体对象

  var localStream = null; // 远端音频流对象

  var remoteAudioStream = null; // 本端视频流禁用状态

  var localVideoMuted = false; // 本端音频流禁用状态
  ...
  function renderRemoteStream(remoteStream) {
    // 远端视频div容器
    var rvsEle = document.querySelector('#rvs');
    ...
    btn.innerText = '关闭视频';
    btn.dataset.muted = 0; // 远端视频上按钮的操作
    ...
    p.innerText = "".concat(remoteStream.userId, "(").concat(remoteStream.display_name, ")");
    var video = document.createElement('video'); // 安卓微信浏览器用

    var x5VideoPlayerFullscreen = document.createAttribute('x5-video-player-fullscreen');
    var x5VideoPlayerType = document.createAttribute('x5-video-player-type');
    x5VideoPlayerFullscreen.value = true;
    x5VideoPlayerType.value = 'h5'; // 设置video属性

    video.setAttributeNode(x5VideoPlayerType);
    video.setAttributeNode(x5VideoPlayerFullscreen);
    video.autoplay = true;
    video.playsInline = true; // 媒体流
  ...
  function initSignalling() {
    ...
    client.on('peer-leave', peerLeave); // 已添加远端流
    ...
    client.on('stream-removed', function (remoteStream) {
      var remoteStreamDivs = document.querySelectorAll('.remote_stream');
    ...
    client.on('local-joined', function (data) {
      ...
      localStream.stream.oninactive = function () {
