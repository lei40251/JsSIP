...
// // clientConfig 配置项
// const configuration = {
//   domain         : 'lccsp.zgpajf.com.cn',
//   wssUrl         : 'wss://lccsp.zgpajf.com.cn:5092/wss',
//   userId         : account,
//   userSig        : account,
//   register       : false,
...
// // Client 实例
// const client = new FlyInn.createConfig(configuration);
...
function createStream(tracks)
{
  ...
  tracks.forEach((track) =>
  {
    if (track.kind === 'video' && track.readyState !== 'ended')
    {
      ...
      track.onended = (s) =>
      {
        for (const item of remoteVideoStreams.values())
        {
...
flyinnUA.on('newRTCSession', (e) =>
{
  ...
  document.querySelector('#muteMic').onclick = () =>
  {
    // 获取视频和麦克风的关闭状态
  ...
  document.querySelector('#muteCam').onclick = () =>
  {
    // 获取视频和麦克风的关闭状态
    curMuted = e.session.isMuted();
    if (curMuted.video)
  ...
  document.querySelector('#camSelect').onchange = () =>
  {
    console.log(this);
  };

  document.querySelector('#switchCam').onclick = () =>
  {
    // 切换摄像头
  ...
  document.querySelector('#screenShare').onclick = () =>
  {
    e.session.displayShare('replace');
  };

  // 呼入振铃 & 呼出回铃音
  e.session.on('progress', (d) =>
  {
  ...
  e.session.on('reinvite', (d) =>
  {
    d.callback = () =>
    {
      ...
      const rTitles = document.querySelectorAll('.r-title');
      ...
      remoteVideoStreams.length>0 && renderRemoteVideos(remoteVideoStreams);
    };
  });

  // 呼叫结束
  e.session.on('ended', (event) =>
  {
    document.querySelector('#video_area').classList = 'hide';
    setStatus('离开会议');
  ...
  e.session.on('sending', (event) =>
  {
    ...
    sid =
      String(crc32(String(new Date().getTime()))) +
...
// 创建会议
document.querySelector('#invite_conf').onclick = () =>
{
  joinConf();
};
...
// 加入会议
document.querySelector('#test_join').onclick = () =>
{
