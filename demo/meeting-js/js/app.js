...
const room = handleGetQuery('room');

const resolution = handleGetQuery('resolution') || 'p480b';

document.title = `房间号:  ${room}`;

document.querySelector('#unmuteMic').style.display = 'none';
document.querySelector('#unmuteCam').style.display = 'none';
...
ua.on('newRTCSession', function(e) 
{
  ...
  e.session.on('sdp', function(d) 
  {
    ...
    if (d.originator === 'local') 
    {
      ...
      d.sdp = d.sdp.replace(/42c01e/g, '428028');
      d.sdp = d.sdp.replace('b=AS:960', 'b=AS:2048');
  ...
  e.session.on('muted', function(d) 
  {
    ...
    else if (d.video) 
    {
      ...
      document.querySelector('#unmuteCam').style.display = 'block';
    }
  });
...
async function call(type, direction) 
{
  ...
  const callee = room || document.querySelector('#callee').value;
...
document.getElementById('small')
  .addEventListener('click', function() 
  {
    ...
    // 获取视频元素
    const smallVideo = smallDiv.querySelector('video');
    const largeVideo = largeDiv.querySelector('video');

    // 交换 srcObject 属性
    const tempStream = largeVideo.srcObject;

    largeVideo.srcObject = smallVideo.srcObject;
    smallVideo.srcObject = tempStream;

    // 临时存储 large div 的内容
    const tempContent = largeText.innerHTML;
