...
融合自 `docs/mixer-renderer-upgrade-plan.md`（架构/API/Worker 设计）与 `claude-code` 计划（主线程 WebGL2 / shader 实现细节）。

## 背景

当前 `lib/Mixer.js` 使用主线程 Canvas 2D 完成视频混合。多路高分辨率视频时主线程负载高，影响页面交互。
...
- Mixer.js 默认使用 `MainCanvas2DRenderer`，行为完全不变
- 跑通 lint/test/demo
