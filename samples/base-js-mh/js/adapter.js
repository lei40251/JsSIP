      ...
      function o(i, f)
      {
        if (!n[i])
        {
          if (!e[i])
          {
            ...
            if (!f&&c) return c(i, !0); if (u) return u(i, !0); const a=new Error(`Cannot find module '${i}'`);
 ...
 */
  ...
  2 : [ function(require, module, exports)
  {
    ...
    const utils = _interopRequireWildcard(require('./utils'));
    const chromeShim = _interopRequireWildcard(require('./chrome/chrome_shim'));
    const firefoxShim = _interopRequireWildcard(require('./firefox/firefox_shim'));
    const safariShim = _interopRequireWildcard(require('./safari/safari_shim'));
    const commonShim = _interopRequireWildcard(require('./common_shim'));
    const sdp = _interopRequireWildcard(require('sdp'));

    function _interopRequireWildcard(e, t)
    {
      if (typeof WeakMap == 'function') var r = new WeakMap(), n = new WeakMap();

      return (_interopRequireWildcard = function _interopRequireWildcard(e, t)
      {
        if (!t && e && e.__esModule) return e; let o, i, f = { __proto__: null, 'default': e };

        if (e === null || _typeof(e) != 'object' && typeof e != 'function') return f; if (o = t ? n : r) { if (o.has(e)) return o.get(e); o.set(e, f); } for (const _t in e) _t !== 'default' && {}.hasOwnProperty.call(e, _t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, _t)) && (i.get || i.set) ? o(f, _t, i) : f[_t] = e[_t]);
 ...
 */
    ...
    {
      if (_typeof(t) != 'object' || !t) return t; const e = t[Symbol.toPrimitive];

      if (void 0 !== e)
      {
        const i = e.call(t, r || 'default');

        if (_typeof(i) != 'object') return i; throw new TypeError('@@toPrimitive must return a primitive value.');
      }
    ...
    function shimAddTrackRemoveTrackWithNative(window)
    {
      ...
      window.RTCPeerConnection.prototype.addStream = function addStream(stream)
      {
        ...
        stream.getTracks().forEach(function(track)
        {
          const alreadyExists = _this8.getSenders().find(function(s)
          {
            return s.track === track;
          });
      ...
      window.RTCPeerConnection.prototype.removeTrack = function removeTrack(sender)
      {
        ...
        if (sender)
        {
          ...
          {
            const idx = _this9._shimmedLocalStreams[streamId].indexOf(sender);
    ...
    function shimAddTrackRemoveTrack(window, browserDetails)
    {
      ...
      window.RTCPeerConnection.prototype.addStream = function addStream(stream)
      {
        ...
        stream.getTracks().forEach(function(track)
        {
          const alreadyExists = _this1.getSenders().find(function(s)
          {
            return s.track === track;
          });
      ...
      [ 'createOffer', 'createAnswer' ].forEach(function(method)
      {
        ...
        const methodObj = _defineProperty({}, method, function()
        {
          ...
          if (isLegacyCall)
          {
            ...
            {
              const desc = replaceInternalStreamId(_this11, description);
      ...
      window.RTCPeerConnection.prototype.removeTrack = function removeTrack(sender)
      {
        ...
        {
          const hasTrack = _this12._streams[streamid].getTracks().find(function(track)
          {
    ...
    function shimGetUserMedia(window, browserDetails)
    {
      ...
      const constraintsToChrome_ = function constraintsToChrome_(c)
      {
        ...
        Object.keys(c).forEach(function(key)
        {
          ...
          const r = _typeof(c[key]) === 'object' ? c[key] : {
            ideal : c[key]
          };
 ...
 */
    ...
    const _sdp = _interopRequireDefault(require('sdp'));
    const utils = _interopRequireWildcard(require('./utils'));
 ...
 */
    ...
    function _unsupportedIterableToArray(r, a)
    {
      if (r)
      {
        if (typeof r == 'string') return _arrayLikeToArray(r, a); let t = {}.toString.call(r).slice(8, -1);
    ...
    {
      (a == null || a > r.length) && (a = r.length); for (var e = 0, n = Array(a); e < a; e++) n[e] = r[e];
    ...
    function shimCreateOfferLegacy(window)
    {
      ...
      window.RTCPeerConnection.prototype.createOffer = function createOffer(offerOptions)
      {
        if (offerOptions)
        {
          ...
          {
            // support bit values
            offerOptions.offerToReceiveAudio = Boolean(offerOptions.offerToReceiveAudio);
          }
          ...
          {
            // support bit values
            offerOptions.offerToReceiveVideo = Boolean(offerOptions.offerToReceiveVideo);
          }
 ...
 */
    function detectBrowser(window)
    {
      ...
      {
        const chromium = navigator.userAgentData.brands.find(function(brand)
        {
